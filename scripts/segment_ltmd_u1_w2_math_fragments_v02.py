#!/usr/bin/env python3
"""Per-viewer, SHA-verified FRAGSEG 0.2 for canonical LTMD-U1 W2 Mathematics."""
from __future__ import annotations
import argparse,csv,hashlib,re,subprocess,tempfile,time,unicodedata
from collections import defaultdict
from pathlib import Path
from urllib.request import Request,urlopen
STRUCTURE=Path('data/catalog/ltmd_u1_w2_math_page_structure.csv');MANIFEST=Path('data/catalog/ltmd_u1_w2_math_reconciled_manifest.csv');VERSION='FRAGSEG_LTMD_U1_W2_MATH_0.2';ELIGIBLE={'textual','mixed_text_image'};UA='LibroTextoMexicanoDigital/U1-W2 Mathematics FRAGSEG 0.2'
...
def download_verify(src,dest,attempts=3):
 ...
 for attempt in range(1,attempts+1):
  try:
   ...
   with urlopen(Request(src['effective_asset_url'],headers={'User-Agent':UA}),timeout=45) as r,dest.open('wb') as f:
    while True:
     b=r.read(1024*1024)
     if not b:break
     h.update(b);total+=len(b);f.write(b)
   if h.hexdigest()!=src['effective_sha256']:raise RuntimeError('effective source SHA mismatch')
   if src.get('effective_byte_size') and total!=int(src['effective_byte_size']):raise RuntimeError('effective source byte-size mismatch')
   return
...
def process_page(r,src,temp):
 ...
 for seq,(text,typ,sig,n) in enumerate(merge_units(units),1):
  if n==0:continue
  fragments.append({'fragment_id':f"{r['page_id']}-F{seq:03d}",'page_id':r['page_id'],'viewer_key':r['viewer_key'],'book_id':r['book_id'],'catalog_generation':r['catalog_generation'],'grade':r['grade'],'viewer_page':p,'fragment_sequence':seq,'candidate_type':typ,'token_count':n,'char_count':len(text),'question_mark_count':sig['q'],'imperative_signal_count':sig['imp'],'material_signal':sig['mat'],'project_signal':sig['proj'],'experiment_signal':sig['exp'],'assessment_signal':sig['assess'],'activity_signal':sig['activity'],'text_sha256':hashlib.sha256(text.encode()).hexdigest(),'segmenter_version':VERSION,'source_structure_class':r['primary_structure'],'classification_certainty':r['classification_certainty'],'uncertain_boundary':int(n>500 or (typ=='other_candidate' and n<4))})
 return fragments,'ok'
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w2_math_fragments');args=ap.parse_args();structure=[r for r in csv.DictReader(STRUCTURE.open(encoding='utf-8')) if r['viewer_key']==args.viewer_key and r['primary_structure'] in ELIGIBLE];sources={(r['viewer_key'],int(r['viewer_page'])):r for r in csv.DictReader(MANIFEST.open(encoding='utf-8')) if r['effective_asset_status'] in ('source_jpeg','source_jpeg_recovered')}
 if not structure:raise SystemExit(f'no eligible PAGESTRUCT rows for {args.viewer_key}')
 allfrags=[];failures=[]
 with tempfile.TemporaryDirectory(prefix='ltmd-u1-w2-math-frag-') as td:
  temp=Path(td)
  for r in structure:
   try:fr,status=process_page(r,sources[(args.viewer_key,int(r['viewer_page']))],temp)
   except Exception as e:fr=[];status=f'exception:{type(e).__name__}'
   if status!='ok' or not fr:failures.append((r['page_id'],r['viewer_key'],r['book_id'],status,len(fr)))
   allfrags.extend(fr)
   for p in temp.iterdir():p.unlink(missing_ok=True)
 d=Path(args.output_dir);d.mkdir(parents=True,exist_ok=True);out=d/f'fragment_{args.viewer_key.lower()}.csv';fail=d/f'fragment_{args.viewer_key.lower()}_failures.csv';fields=['fragment_id','page_id','viewer_key','book_id','catalog_generation','grade','viewer_page','fragment_sequence','candidate_type','token_count','char_count','question_mark_count','imperative_signal_count','material_signal','project_signal','experiment_signal','assessment_signal','activity_signal','text_sha256','segmenter_version','source_structure_class','classification_certainty','uncertain_boundary']
 with out.open('w',encoding='utf-8',newline='') as f:w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(allfrags)
 with fail.open('w',encoding='utf-8',newline='') as f:w=csv.writer(f);w.writerow(['page_id','viewer_key','book_id','status','fragment_count']);w.writerows(failures)
 fatal=[x for x in failures if x[3]!='ok'];print(f'{args.viewer_key}: eligible_pages={len(structure)} fragments={len(allfrags)} empty_pages={len(failures)-len(fatal)} fatal_failures={len(fatal)}')
 if fatal:raise SystemExit(f'{args.viewer_key}: {len(fatal)} source/OCR execution failures; refusing shard publication')
