...
OUT=Path('data/catalog/ltmd_u1_w9_viewer_architecture.csv')
REPORT=Path('data/catalog/ltmd_u1_w9_viewer_architecture.md')
VERSION='LTMD_U1_W9_ARCHITECTURE_0.1'
UA='LibroTextoMexicanoDigital/U1-W9 Physical Education architecture audit'
EXPECTED=4
...
def main():
    ...
    if len(scope)!=EXPECTED or len({r['viewer_key'] for r in scope})!=EXPECTED:raise SystemExit(f'W9 architecture: expected {EXPECTED} scope rows')
    rows=[]
    for s in scope:
        html_status,html=get(s['source_url']);base=s['source_url'].rsplit('/',1)[0]+'/'
        has_x=int(bool(re.search(r'(?:src=["\'][^"\']*/)?x\.js(?:[?"\'])',html,re.I)) or 'x.js' in html)
        x_status,xjs=get(base+'x.js') if has_x else (0,'');ag=int('ag_pages' in xjs)
    ...
    lines=['# LTMD-U1 W9 — arquitectura de visores Educación Física','',f'Versión: `{VERSION}`.','',f'- Visores congelados: **{EXPECTED}**.',f'- HTML 200: **{html_ok}/{EXPECTED}**.',f'- `x.js` 200: **{x_ok}/{EXPECTED}**.',f'- señal `ag_pages`: **{ag_ok}/{EXPECTED}**.',f'- arquitectura dinámica estándar: **{standard}/{EXPECTED}**.',f'- casos no estándar: **{EXPECTED-standard}**.','','Este probe verifica únicamente la arquitectura necesaria para auditar inventario declarado y activos institucionales. No acredita disponibilidad página por página ni equivalencia entre libros.','','Un visor HTML accesible no se considera por sí mismo fuente admitida para OCR.']
