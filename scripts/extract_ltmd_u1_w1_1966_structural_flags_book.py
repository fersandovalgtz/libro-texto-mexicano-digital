#!/usr/bin/env python3
"""Extract front/end structural keyword flags for one LTMD-U1 W1 1966 book."""
...
UA='LibroTextoMexicanoDigital/U1-W1 1966 structural flags'
VOCAB={
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--book-id',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w1_1966_structkw');args=ap.parse_args()
    metrics=[r for r in csv.DictReader(METRICS.open(encoding='utf-8')) if r['book_id']==args.book_id]
    if not metrics:raise SystemExit(f'no W1 1966 OCR rows for {args.book_id}')
    manifest={r['page_id']:r for r in csv.DictReader(MAN.open(encoding='utf-8')) if r['book_id']==args.book_id and r['asset_status']=='source_jpeg'}
    ...
    out=[]
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w1-1966-struct-') as td:
        td=Path(td)
        for r in candidates:
    ...
    d=Path(args.output_dir);d.mkdir(parents=True,exist_ok=True);slug=args.book_id.lower().replace('u1-','');p=d/f'structkw_{slug}.csv'
