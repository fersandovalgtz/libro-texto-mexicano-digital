#!/usr/bin/env python3
"""Classify canonical W4 Social Sciences pages with conservative PAGESTRUCT logic."""
from __future__ import annotations
import csv
from collections import Counter,defaultdict
from pathlib import Path
METRICS=Path('data/catalog/ltmd_u1_w4_social_sciences_ocr_metrics.csv');FLAGS=Path('data/catalog/ltmd_u1_w4_social_sciences_structural_keyword_flags.csv');OUT=Path('data/catalog/ltmd_u1_w4_social_sciences_page_structure.csv');SUMMARY=Path('data/catalog/ltmd_u1_w4_social_sciences_page_structure_summary.csv');REPORT=Path('data/catalog/ltmd_u1_w4_social_sciences_page_structure.md');VERSION='PAGESTRUCT_LTMD_U1_W4_SOCIAL_SCIENCES_0.1';EXPECTED=14;EXPECTED_PAGES=2414
...
def main():
 ...
 if len(metrics)!=EXPECTED_PAGES or len({r['viewer_key'] for r in metrics})!=EXPECTED:raise SystemExit('W4 PAGESTRUCT OCR coverage mismatch')
 out=[]
 for r in metrics:
 ...
 allc=counts['ALL'];eligible=allc['textual']+allc['mixed_text_image'];lines=['# PAGESTRUCT — LTMD-U1 W4 Ciencias Sociales','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','',f'Objetos canónicos: **{EXPECTED}**.','','## Total']
 for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
 lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','','## Regla','Se conserva la lógica PAGESTRUCT conservadora. Esta capa es estructural, no semántica. La ausencia de referencia humana no impide PAGESTRUCT/FRAGSEG, pero sí impide tratar clasificadores semánticos no validados como evidencia histórica primaria.']
