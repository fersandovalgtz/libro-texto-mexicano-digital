#!/usr/bin/env python3
"""Classify canonical W6 Geography/Atlas pages with conservative PAGESTRUCT logic."""
from __future__ import annotations
import csv
from collections import Counter,defaultdict
from pathlib import Path
METRICS=Path('data/catalog/ltmd_u1_w6_geography_atlas_ocr_metrics.csv');FLAGS=Path('data/catalog/ltmd_u1_w6_geography_atlas_structural_keyword_flags.csv');OUT=Path('data/catalog/ltmd_u1_w6_geography_atlas_page_structure.csv');SUMMARY=Path('data/catalog/ltmd_u1_w6_geography_atlas_page_structure_summary.csv');REPORT=Path('data/catalog/ltmd_u1_w6_geography_atlas_page_structure.md');VERSION='PAGESTRUCT_LTMD_U1_W6_GEOGRAPHY_ATLAS_0.1';EXPECTED=37;EXPECTED_PAGES=5258
...
def main():
 ...
 if len(metrics)!=EXPECTED_PAGES or len({r['viewer_key'] for r in metrics})!=EXPECTED:raise SystemExit('W6 PAGESTRUCT OCR coverage mismatch')
 out=[]
 for r in metrics:
 ...
 allc=counts['ALL'];eligible=allc['textual']+allc['mixed_text_image'];lines=['# PAGESTRUCT — LTMD-U1 W6 Geografía/Atlas','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','',f'Objetos canónicos: **{EXPECTED}**.','','## Total']
 for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
 lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','','## Regla','Se conserva sin cambios la lógica PAGESTRUCT conservadora usada en olas anteriores. Esta capa es estructural, no semántica; `WAITING_HUMAN_REFERENCE` permanece vigente.']
