#!/usr/bin/env python3
"""Combine W6 Geography/Atlas FRAGSEG shards with exact coverage checks."""
from __future__ import annotations
import argparse,csv
from collections import Counter,defaultdict
from pathlib import Path
STRUCT=Path('data/catalog/ltmd_u1_w6_geography_atlas_page_structure.csv');OUT=Path('data/catalog/ltmd_u1_w6_geography_atlas_fragment_manifest.csv');SUMMARY=Path('data/catalog/ltmd_u1_w6_geography_atlas_fragment_manifest_summary.csv');GAPS=Path('data/catalog/ltmd_u1_w6_geography_atlas_fragment_sequence_gaps.csv');REPORT=Path('data/catalog/ltmd_u1_w6_geography_atlas_fragment_manifest.md');VERSION='FRAGSEG_LTMD_U1_W6_GEOGRAPHY_ATLAS_0.1';ELIGIBLE={'textual','mixed_text_image'};EXPECTED=37

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w6_geography_atlas_fragments');a=ap.parse_args();structure=list(csv.DictReader(STRUCT.open(encoding='utf-8',newline='')));viewers={r['viewer_key'] for r in structure};eligible={(r['viewer_key'],r['page_id']) for r in structure if r['primary_structure'] in ELIGIBLE}
 if len(structure)!=5258 or len(viewers)!=EXPECTED:raise SystemExit(f'W6 PAGESTRUCT coverage mismatch rows={len(structure)} viewers={len(viewers)}')
 ...
 for p in failfiles:
  ...
  for r in rr:
   if r['status']!='ok':raise SystemExit(f'fatal W6 FRAGSEG failure persisted: {r}')
 ...
 for p in files:
  ...
  if len(keys)!=1:raise SystemExit(f'cannot identify exactly one W6 viewer for shard {p}')
  key=next(iter(keys));seen.append(key)
  if rr and {r['segmenter_version'] for r in rr}!={VERSION}:raise SystemExit(f'invalid W6 segmenter version {p}')
  rows+=rr
 if set(seen)!=viewers or len(seen)!=EXPECTED or len(seen)!=len(set(seen)):raise SystemExit('W6 FRAGSEG viewer coverage mismatch')
 ...
 if pagekeys!=eligible:raise SystemExit(f'W6 eligible coverage mismatch missing={len(eligible-pagekeys)} extra={len(pagekeys-eligible)}')
 bypage=defaultdict(list)
 for r in rows:bypage[(r['viewer_key'],r['page_id'])].append(int(r['fragment_sequence']))
 gaprows=[]
 for (k,pid),vals in sorted(bypage.items()):
  seq=sorted(vals)
  if any(v<=0 for v in seq) or len(seq)!=len(set(seq)):raise SystemExit(f'invalid W6 fragment sequence {k} {pid}')
 ...
 if not rows:raise SystemExit('W6 FRAGSEG produced zero fragments')
 ...
 allc=summary[-1];slots=sum(int(r['missing_slot_count']) for r in gaprows);lines=['# FRAGSEG — LTMD-U1 W6 Geografía/Atlas','',f'Versión: `{VERSION}`.','',f'- Objetos canónicos: **{EXPECTED}**.',f'- Páginas elegibles PAGESTRUCT: **{len(eligible):,}**.',f'- Páginas con ≥1 fragmento: **{allc["segmented_page_count"]:,}**.',f'- Páginas elegibles sin fragmentos: **{len(empty)}**.',f'- Fragmentos: **{allc["fragment_count"]:,}**.',f'- IDs únicos: **{len(set(ids)):,}**.',f'- Páginas con huecos legítimos de secuencia: **{len(gaprows)}**.',f'- Slots omitidos: **{slots}**.','','## Tipos candidatos']
 for t in types:lines.append(f'- `{t}`: {allc[t]:,}.')
 lines+=['','## Regla','Se reutiliza sin cambios el motor FRAGSEG W3 para comparabilidad técnica. Los cinco visores 2018 permanecen representados por aliases de provenance hacia 2019 y no generan fragmentación duplicada. Las dos páginas 2008 recuperadas usan sus fuentes efectivas criptográficamente verificadas. Los tipos son candidatos técnicos; esta capa no constituye validación semántica y `WAITING_HUMAN_REFERENCE` permanece vigente.']
