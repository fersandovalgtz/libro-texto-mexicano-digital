#!/usr/bin/env python3
"""Audit one LTMD-U1 W5 History viewer from institutional ag_clave.
...
INV=Path('data/catalog/ltmd_u1_w5_declared_inventory.csv')
ARCH=Path('data/catalog/ltmd_u1_w5_viewer_architecture.csv')
VERSION='LTMD_U1_W5_HISTORY_ASSET_AUDIT_0.1'
BASE='https://historico.conaliteg.gob.mx/c/{key}/{idx:03d}.jpg'
UA='LibroTextoMexicanoDigital/U1-W5 History asset audit'
EXPECTED=18
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w5_history_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W5 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W5: {a.viewer_key}')
