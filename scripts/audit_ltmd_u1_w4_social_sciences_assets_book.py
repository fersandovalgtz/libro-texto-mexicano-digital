#!/usr/bin/env python3
"""Audit one LTMD-U1 W4 Social Sciences viewer from institutional ag_clave.
...
INV=Path('data/catalog/ltmd_u1_w4_declared_inventory.csv')
ARCH=Path('data/catalog/ltmd_u1_w4_viewer_architecture.csv')
VERSION='LTMD_U1_W4_SOCIAL_SCIENCES_ASSET_AUDIT_0.1'
BASE='https://historico.conaliteg.gob.mx/c/{key}/{idx:03d}.jpg'
UA='LibroTextoMexicanoDigital/U1-W4 Social Sciences asset audit'
EXPECTED=14
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w4_social_sciences_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W4 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W4: {a.viewer_key}')
