...
METRICS=Path('data/catalog/ltmd_u1_w2_math_ocr_metrics.csv');OUT=Path('data/catalog/ltmd_u1_w2_math_structural_keyword_flags.csv');VERSION='LTMD_U1_W2_MATH_STRUCTKW_0.1';EXPECTED=64

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w2_math_structkw');a=ap.parse_args();metrics=list(csv.DictReader(METRICS.open(encoding='utf-8')));by={}
 for r in metrics:by.setdefault(r['viewer_key'],[]).append(r)
 if len(by)!=EXPECTED:raise SystemExit(f'expected {EXPECTED} OCR viewers, found {len(by)}')
 expected=set()
 for key,rr in by.items():
  m=max(int(r['viewer_page']) for r in rr);expected|={(key,r['page_id']) for r in rr if int(r['viewer_page'])<=16 or int(r['viewer_page'])>m-16}
 ...
 if set(seen)!=set(by) or len(seen)!=EXPECTED:raise SystemExit('structural viewer coverage mismatch')
 ...
 rows.sort(key=lambda r:(int(r['catalog_generation']),int(r['grade']),r['viewer_key'],int(r['viewer_page'])));OUT.parent.mkdir(parents=True,exist_ok=True)
 with OUT.open('w',encoding='utf-8',newline='') as f:w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
 print(f'wrote {len(rows)} W2 Mathematics structural rows for {EXPECTED} viewers; all SHA verified')
