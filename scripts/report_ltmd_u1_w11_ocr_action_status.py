...
VERSION='LTMD_U1_W11_OCR_RUN_STATUS_0.4';WORKFLOW='build-ltmd-u1-w11-ocr.yml';API='https://api.github.com';EXPECTED_MATRIX=106

def api(path,token,params=None,attempts=3):
    url=API+path+('?' + urlencode(params) if params else '');last=''
    for attempt in range(1,attempts+1):
        try:
            req=Request(url,headers={'Authorization':f'Bearer {token}','Accept':'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','User-Agent':'LTMD-W11-status-reporter/0.4'})
...
def main():
    repo=os.environ['GITHUB_REPOSITORY'];token=os.environ['GITHUB_TOKEN'];observer_run=int(os.environ.get('GITHUB_RUN_ID','0') or 0);observed=datetime.now(timezone.utc).isoformat().replace('+00:00','Z')
    observer_jobs=jobs_for_run(repo,observer_run,token) if observer_run else []
    observer_job=next((j for j in observer_jobs if j.get('name')=='report'),None)
    ...
    matrix=matrix_jobs(jobs);status=Counter(j.get('status') or 'unknown' for j in jobs)
    ...
    summary={'report_version':VERSION,'observed_at_utc':observed,'observer_run_id':observer_run or None,'observer_job_id':observer_job.get('id') if observer_job else None,'repository':repo,'workflow':WORKFLOW,'selected_run_id':selected['id'],'status':selected.get('status',''),'conclusion':selected.get('conclusion'),'head_sha':selected.get('head_sha',''),'matrix_expected':EXPECTED_MATRIX,'matrix_jobs_detected':len(matrix),'matrix_success':len(success),'matrix_failure':len(failed),'matrix_cancelled':len(cancelled),'matrix_active':len(active_matrix),'progress_percent':round(pct,2),'failed_jobs':[j.get('name','') for j in failed],'cancelled_jobs':[j.get('name','') for j in cancelled],'job_status_counts':dict(status),'active_runs_inspected':[{'id':r['id'],'status':r.get('status',''),'jobs':len(jj),'matrix_jobs':m} for r,jj,m in inspected]}
    JSON_OUT.write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    lines=['# LTMD-U1 W11 — estado operativo del OCR','',f'Versión: `{VERSION}`. Observado: **{observed}**.','', '> Estado operativo solamente; G5 cierra únicamente con el OCR consolidado validado.','',f'- Run observador: **{observer_run or "—"}**; job observador: **{observer_job.get("id") if observer_job else "—"}**.',f'- Run con matriz: **{selected["id"]}**.',f'- Matriz: **{len(matrix)}/{EXPECTED_MATRIX}** jobs.',f'- Exitosos: **{len(success)}/{EXPECTED_MATRIX} ({pct:.2f}%)**.',f'- Fallidos: **{len(failed)}**.',f'- Cancelados: **{len(cancelled)}**.',f'- Aún no completados: **{len(active_matrix)}**.','','## Estado global de jobs']
    ...
    lines+=['','## Regla','El job observador puede reejecutarse para obtener un nuevo corte sin tocar OCR. No se mezclan shards entre runs. Sólo el combine del mismo run puede demostrar 19,862/19,862 páginas, SHA verificado y cero `unresolved`.']
