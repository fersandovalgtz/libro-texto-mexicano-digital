...
OUT=Path('data/catalog/ltmd_u1_w7_viewer_architecture.csv')
REPORT=Path('data/catalog/ltmd_u1_w7_viewer_architecture.md')
VERSION='LTMD_U1_W7_ARCHITECTURE_0.1'
UA='LibroTextoMexicanoDigital/U1-W7 Civics Ethics architecture audit'
EXPECTED=30
...
def main():
    ...
    lines=['# LTMD-U1 W7 — arquitectura de visores Cívica/Ética','',f'Versión: `{VERSION}`.','',f'- Visores congelados: **{EXPECTED}**.',f'- HTML 200: **{html}/{EXPECTED}**.',f'- `x.js` 200: **{xok}/{EXPECTED}**.',f'- señal `ag_pages`: **{ag}/{EXPECTED}**.',f'- arquitectura dinámica estándar: **{standard}/{EXPECTED}**.',f'- casos no estándar: **{EXPECTED-standard}**.','','## Por generación','', '| generación | visores | estándar |','|---:|---:|---:|']
    for g,(n,st) in sorted(gens.items(),key=lambda x:int(x[0])):lines.append(f'| {g} | {n} | {st} |')
    lines += ['', 'Este probe verifica solamente la arquitectura necesaria para auditar conteos declarados y activos JPEG. No acredita disponibilidad página por página, identidad entre generaciones ni continuidad curricular. OCR permanece cerrado hasta reconciliación de fuente.']
