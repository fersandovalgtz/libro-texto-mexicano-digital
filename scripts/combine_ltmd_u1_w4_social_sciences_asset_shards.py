...
DECL=Path('data/catalog/ltmd_u1_w4_declared_inventory.csv')
OUT=Path('data/catalog/ltmd_u1_w4_social_sciences_asset_manifest.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w4_social_sciences_asset_summary.csv')
REPORT=Path('data/catalog/ltmd_u1_w4_social_sciences_asset_audit.md')
VERSION='LTMD_U1_W4_SOCIAL_SCIENCES_ASSET_AUDIT_0.1'
EXPECTED=14

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w4_social_sciences_assets');a=ap.parse_args()
    ...
    if len(scope)!=EXPECTED or len(decl)!=EXPECTED or len(files)!=EXPECTED:raise SystemExit(f'W4 scope/declared/files mismatch {len(scope)}/{len(decl)}/{len(files)}')
    rows=[];seen=[]
    for p in files:
        ...
        if len(ks)!=1 or vs!={VERSION}:raise SystemExit(f'invalid W4 shard {p}')
        key=next(iter(ks));seen.append(key);expected=int(decl[key]['declared_positions'])
        if len(rr)!=expected:raise SystemExit(f'W4 cardinality mismatch {key}: {len(rr)} vs {expected}')
        if any(r['probe_state']=='probe_error' or r['asset_status']=='probe_error' for r in rr):raise SystemExit(f'W4 probe error persisted {key}')
        if {r['ag_clave'] for r in rr}!={decl[key]['ag_clave']}:raise SystemExit(f'W4 ag_clave drift {key}')
        rows+=rr
    if set(seen)!=set(scope) or len(seen)!=len(set(seen)):raise SystemExit('W4 viewer coverage/duplicate mismatch')
    ...
    lines += ['', '## Regla', 'Cada byte servido se recorre sólo para SHA-256 y tamaño; no se persisten JPEG. Un 404 final se conserva como candidato terminal y un 404 interno como anomalía. `direct_asset_ready` es un estado técnico de fuente y no acredita independencia documental ni equivalencia curricular.', '', 'OCR W4 permanece cerrado hasta analizar aliases exactos, routing anómalo y huecos internos.']
