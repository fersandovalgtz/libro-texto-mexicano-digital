#!/usr/bin/env python3
"""SHA-verified adaptive OCR metrics for one canonical LTMD-U1 W6 Geography/Atlas viewer."""
from __future__ import annotations
import argparse,csv,hashlib,os,statistics,subprocess,tempfile
from pathlib import Path
from urllib.request import Request,urlopen

MAN=Path('data/catalog/ltmd_u1_w6_geography_atlas_canonical_page_manifest.csv')
PROC=Path('data/catalog/ltmd_u1_w6_geography_atlas_processing_inventory.csv')
VERSION='LTMD_U1_W6_GEOGRAPHY_ATLAS_OCR_0.1'
UA='LibroTextoMexicanoDigital/U1-W6 Geography Atlas OCR 0.1'
FALLBACK_MIN_WORDS=5;TIMEOUT=60
EXPECTED_IDENTITIES=42;EXPECTED_CANONICAL=37;EXPECTED_ALIASES=5;EXPECTED_SOURCE_PAGES=5258
FIELDS=['ocr_version','page_id','viewer_key','catalog_generation','grade','title_core','viewer_page','processing_mode','source_provenance','source_bytes','source_sha256_verified','attempts','selected_psm','recognized_words','ocr_chars','mean_word_confidence','median_word_confidence','low_confidence_word_rate','ocr_class','ocr_status','error']

def load_topology():
    proc=list(csv.DictReader(PROC.open(encoding='utf-8',newline='')));man=list(csv.DictReader(MAN.open(encoding='utf-8',newline='')))
    if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:raise SystemExit('W6 processing inventory cardinality mismatch')
    if any(r['technical_identity_covered']!='1' or int(r['persistent_source_gaps'])!=0 for r in proc):raise SystemExit('W6 topology contains uncovered identity or persistent gap')
    ...
    if any(r['processing_mode'] not in {'direct_canonical','direct_canonical_reconciled_gap'} for r in proc if r['viewer_key'] in canonical):raise SystemExit('W6 canonical processing mode mismatch')
    if len(man)!=EXPECTED_SOURCE_PAGES or {r['viewer_key'] for r in man}!=canonical:raise SystemExit('W6 canonical page manifest mismatch')
    if len({r['page_id'] for r in man})!=EXPECTED_SOURCE_PAGES:raise SystemExit('duplicate W6 canonical page IDs')
    if sum(r['source_kind']=='cryptographically_recovered_same_position_reference' for r in man)!=2:raise SystemExit('W6 recovered-page count mismatch')
    return canonical,{r['viewer_key']:r for r in proc}
...
def process(row,proc,tmp):
    image=tmp/f"{row['viewer_key']}_{int(row['viewer_page']):04d}.jpg";attempts=[];errors=[]
    base={'ocr_version':VERSION,'page_id':row['page_id'],'viewer_key':row['viewer_key'],'catalog_generation':row['catalog_generation'],'grade':row['grade_code'],'title_core':row['title_core'],'viewer_page':row['viewer_page'],'processing_mode':proc['processing_mode'],'source_provenance':row['source_kind']};empty={'recognized_words':'','ocr_chars':'','mean_word_confidence':'','median_word_confidence':'','low_confidence_word_rate':''}
    try:
        ...
        if baseline and int(baseline['recognized_words'])>0:return {**base,'source_bytes':size,'source_sha256_verified':1,'attempts':';'.join(attempts),'selected_psm':3,**baseline,'ocr_class':'text_detected','ocr_status':'ok','error':' | '.join(errors)}
        fallback=[]
        for psm in (11,6):
    ...
    except Exception as exc:return {**base,'source_bytes':'','source_sha256_verified':0,'attempts':';'.join(attempts),'selected_psm':'',**empty,'ocr_class':'unresolved','ocr_status':'error','error':f'{type(exc).__name__}: {exc}'}
    finally:image.unlink(missing_ok=True)

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w6_geography_atlas_ocr');a=ap.parse_args();canonical,proc=load_topology()
    if a.viewer_key not in canonical:raise SystemExit(f'viewer not W6 canonical: {a.viewer_key}')
    source=[r for r in csv.DictReader(MAN.open(encoding='utf-8',newline='')) if r['viewer_key']==a.viewer_key]
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w6-geo-ocr-') as td:rows=[process(r,proc[a.viewer_key],Path(td)) for r in source]
