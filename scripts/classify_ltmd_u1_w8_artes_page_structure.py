#!/usr/bin/env python3
"""Classify canonical W8 Artes pages with the frozen conservative PAGESTRUCT logic."""
from __future__ import annotations
import csv
from collections import Counter,defaultdict
from pathlib import Path
METRICS=Path('data/catalog/ltmd_u1_w8_artes_ocr_metrics.csv');FLAGS=Path('data/catalog/ltmd_u1_w8_artes_structural_keyword_flags.csv');OUT=Path('data/catalog/ltmd_u1_w8_artes_page_structure.csv');SUMMARY=Path('data/catalog/ltmd_u1_w8_artes_page_structure_summary.csv');REPORT=Path('data/catalog/ltmd_u1_w8_artes_page_structure.md');VERSION='PAGESTRUCT_LTMD_U1_W8_ARTES_0.1';EXPECTED=16;EXPECTED_PAGES=1490
...
def main():
 ...
 if len(metrics)!=EXPECTED_PAGES or len({r['viewer_key'] for r in metrics})!=EXPECTED:raise SystemExit('W8 PAGESTRUCT OCR coverage mismatch')
 if any(r['source_sha256_verified']!='1' or r['ocr_status']!='ok' for r in metrics):raise SystemExit('W8 PAGESTRUCT refuses unverified/unresolved OCR rows')
 out=[]
 for r in metrics:
 ...
 allc=counts['ALL'];eligible=allc['textual']+allc['mixed_text_image'];lines=['# PAGESTRUCT — LTMD-U1 W8 Artes','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','',f'Objetos canónicos: **{EXPECTED}**.','Identidades históricas retenidas por fuente y excluidas de PAGESTRUCT: **4**.','','## Total']
 for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
 lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','','## Regla','Se conserva sin cambios la lógica PAGESTRUCT conservadora empleada en W4/W7 para mantener comparabilidad técnica. Esta capa es estructural, no semántica. Las cuatro identidades 2018 retenidas por fuente permanecen fuera de esta cohorte y no se imputan ni sustituyen. La ausencia de referencia humana no impide PAGESTRUCT/FRAGSEG, pero sí impide tratar clasificadores semánticos no validados como evidencia histórica primaria.']
