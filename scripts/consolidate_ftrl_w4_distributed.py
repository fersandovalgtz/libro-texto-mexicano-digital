#!/usr/bin/env python3
"""Privately consolidate decrypted distributed W4 FTRL shards."""
...
EXPECTED_SHARDS = 8
SCHEMA = "LTMD_FTRL_W4_PRIVATE_CONSOLIDATION_0.1"
...
def load_records(root: Path, shard_count: int) -> list[dict]:
    paths = sorted(root.rglob("w4_shard_*_page_ocr.jsonl"))
    if len(paths) != shard_count:
        raise SystemExit(f"expected {shard_count} decrypted W4 JSONL shards, found {len(paths)}")
    records: list[dict] = []
    page_ids = set()
    page_keys = set()
    for path in paths:
        with path.open(encoding="utf-8") as fh:
            for line_number, line in enumerate(fh, 1):
                ...
                row = json.loads(line)
                if row.get("wave") != "W4":
                    raise SystemExit(f"non-W4 record in {path}:{line_number}")
                page_id = str(row["page_id"])
                key = (str(row["viewer_key"]), int(row["page_index"]))
                if page_id in page_ids or key in page_keys:
                    raise SystemExit(f"duplicate W4 page across shards: {page_id}")
                page_ids.add(page_id)
                page_keys.add(key)
                records.append(row)
    if len(records) != EXPECTED_PAGES:
        raise SystemExit(f"W4 decrypted distributed total drift: {len(records)}")
...
def main() -> None:
    ...
    if observed != expected:
        raise SystemExit(f"W4 private union mismatch: missing={len(expected-observed)}, extra={len(observed-expected)}")

    out = args.output_dir
    out.mkdir(parents=True, exist_ok=True)
    jsonl = out / "ltmd_u1_w4_full_page_ocr.jsonl"
    db = out / "ltmd_u1_w4_full_ocr_search.sqlite"
    qc_queue = out / "ltmd_u1_w4_full_qc_queue.json"
    qc_summary = out / "ltmd_u1_w4_full_qc_summary.json"
    run_manifest = out / "ltmd_u1_w4_full_run_manifest.json"
    evidence = out / "ltmd_u1_w4_private_consolidation_evidence.json"
    ...
    if rm["status"] != "validated" or rm["corpus"]["page_records"] != EXPECTED_PAGES:
        raise SystemExit("W4 private consolidated run failed")
    if rm["database"]["page_rows"] != EXPECTED_PAGES or rm["database"]["fts_rows"] != EXPECTED_PAGES or rm["database"]["sqlite_integrity"] != "ok" or qc["page_records"] != EXPECTED_PAGES:
        raise SystemExit("W4 private consolidated database/QC failure")
    if rm["database"]["historical_identities"] != EXPECTED_HISTORICAL:
        raise SystemExit(f"W4 historical identity drift: {rm['database']['historical_identities']}")
