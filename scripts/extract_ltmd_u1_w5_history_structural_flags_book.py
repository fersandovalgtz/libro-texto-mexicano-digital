#!/usr/bin/env python3
"""Extract conservative structural keyword flags for one canonical LTMD-U1 W5 History viewer."""
from __future__ import annotations
import argparse,csv,hashlib,os,re,subprocess,tempfile,unicodedata
from pathlib import Path
from urllib.request import Request,urlopen

METRICS=Path('data/catalog/ltmd_u1_w5_history_ocr_metrics.csv')
MAN=Path('data/catalog/ltmd_u1_w5_history_canonical_page_manifest.csv')
VERSION='LTMD_U1_W5_HISTORY_STRUCTKW_0.1'
UA='LibroTextoMexicanoDigital/U1-W5 History structural flags 0.1'
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w5_history_structkw');a=ap.parse_args()
 metrics_all=list(csv.DictReader(METRICS.open(encoding='utf-8',newline='')))
 if len({r['viewer_key'] for r in metrics_all})!=EXPECTED_CANONICAL:raise SystemExit('W5 structural flags require 15 canonical OCR viewers')
 metrics=[r for r in metrics_all if r['viewer_key']==a.viewer_key]
 if not metrics:raise SystemExit(f'no W5 OCR rows for {a.viewer_key}')
 ...
 with tempfile.TemporaryDirectory(prefix='ltmd-u1-w5-history-struct-') as td:
  td=Path(td)
