#!/usr/bin/env python3
"""Audit one W10 viewer position-by-position, hashing served official images without persisting them."""
...
UA='LibroTextoMexicanoDigital/U1-W10 integrated-multiarea asset audit 0.1'
EXPECTED=69
...
def fetch_hash(url,attempts=3):
    ...
    for attempt in range(1,attempts+1):
        try:
            ...
            if e.code==404:
                return {'probe_state':'http_404','http_status':404,'content_type':e.headers.get('Content-Type','') if e.headers else '','byte_size':'','sha256':'','attempts':attempt,'error':'HTTP 404'}
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w10_integrados_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W10 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W10: {a.viewer_key}')
    m=inv[a.viewer_key];ar=arch[a.viewer_key];n=int(m['declared_positions']);key=m['ag_clave']
    if ar['standard_dynamic_architecture']!='1':raise SystemExit(f'nonstandard architecture requires separate route audit: {a.viewer_key}')
    ...
    for page in range(1,n+1):
        ...
        records.append({'audit_version':VERSION,'viewer_key':a.viewer_key,'catalog_generation':m['catalog_generation'],'grade_code':m['grade_code'],'title_core':m['title_core'],'viewer_ui':'standard_x_js','ag_clave':key,'viewer_page':page,'declared_positions':n,'source_image_index':idx,'source_asset_url':url,'is_final_declared_position':int(page==n),**p})
