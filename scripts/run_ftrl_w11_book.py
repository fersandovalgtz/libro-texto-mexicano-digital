#!/usr/bin/env python3
"""Run one deterministic LTMD-U1 W11 Otros/No clasificados FTRL book unit.
...
SCHEMA = "LTMD_FTRL_W11_BOOK_UNIT_0.1"
    ...
    with path.open(encoding="utf-8") as fh:
        for line_number, line in enumerate(fh, 1):
            if not line.strip():
                continue
            row = json.loads(line)
            if row.get("wave") != "W11" or row.get("viewer_key") != viewer_key:
                raise SystemExit(f"foreign W11 record at {path}:{line_number}")
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", type=Path, default=Path("local/ftrl-w11"))
    args = ap.parse_args()
    require_environment()
    ...
    full_asset = root / "ltmd_u1_w11_asset_manifest.csv"
    processing = root / "ltmd_u1_w11_processing_inventory.csv"
    run([
        sys.executable, "scripts/build_ftrl_w11_inputs.py",
    ...
    if len(proc) != EXPECTED_CANONICAL:
        raise SystemExit("W11 canonical processing denominator drift")
    canonical = {
    ...
    if len(canonical) != EXPECTED_CANONICAL or viewer not in canonical:
        raise SystemExit(f"viewer is not a W11 source-admitted canonical identity: {viewer}")
    if any(r["persistent_internal_source_gaps"] != "0" for r in proc):
        raise SystemExit("W11 admitted processing inventory contains a source gap")
    ...
    if len(assets) != EXPECTED_TOTAL:
        raise SystemExit("W11 global source cardinality drift")
    if len({r["viewer_key"] for r in assets}) != EXPECTED_CANONICAL:
        raise SystemExit("W11 source-admitted canonical viewer denominator drift")
    rows = sorted(
    ...
    if len(rows) != pages_expected:
        raise SystemExit(
            f"W11 {viewer} source cardinality drift: {len(rows)} != {pages_expected}"
        )
    ...
    if len(expected_hashes) != len(set(expected_hashes)):
        raise SystemExit("duplicate expected page key inside W11 book unit")

    prefix = f"w11_{viewer}"
    ...
    if actual_count != pages_expected or actual_hashes != expected_hashes:
        raise SystemExit("W11 book page inventory mismatch")
    ...
    if rm["status"] != "validated" or rm["database"]["sqlite_integrity"] != "ok":
        raise SystemExit("W11 book validation failed")
    if (
        ...
        raise SystemExit("W11 book cardinality drift")
