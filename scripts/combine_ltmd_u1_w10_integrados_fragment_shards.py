#!/usr/bin/env python3
"""Combine W10 FRAGSEG shards with exact topology/page coverage checks."""
from __future__ import annotations
import argparse,csv
from collections import Counter,defaultdict
from pathlib import Path
STRUCT=Path('data/catalog/ltmd_u1_w10_integrados_page_structure.csv');PROC=Path('data/catalog/ltmd_u1_w10_processing_inventory.csv');OUT=Path('data/catalog/ltmd_u1_w10_integrados_fragment_manifest.csv');SUMMARY=Path('data/catalog/ltmd_u1_w10_integrados_fragment_manifest_summary.csv');GAPS=Path('data/catalog/ltmd_u1_w10_integrados_fragment_sequence_gaps.csv');REPORT=Path('docs/LTMD_U1_W10_FRAGSEG.md');VERSION='FRAGSEG_LTMD_U1_W10_INTEGRADOS_0.1';STRUCT_VERSION='PAGESTRUCT_LTMD_U1_W10_INTEGRADOS_0.1';ELIGIBLE={'textual','mixed_text_image'};EXPECTED_IDENTITIES=69

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w10_integrados_fragments');a=ap.parse_args();structure=list(csv.DictReader(STRUCT.open(encoding='utf-8',newline='')));proc=list(csv.DictReader(PROC.open(encoding='utf-8',newline='')))
    if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:raise SystemExit('W10 FRAGSEG topology cardinality mismatch')
    ...
    if not structure or viewers!=canonical or {r['classifier_version'] for r in structure}!={STRUCT_VERSION}:raise SystemExit('W10 FRAGSEG PAGESTRUCT coverage/version mismatch')
    per={k:0 for k in canonical}
    for k,_ in eligible:per[k]+=1
    bad=[k for k,n in per.items() if n==0]
    if bad:raise SystemExit(f'W10 canonical viewers without FRAGSEG-eligible pages: {bad}')
    ...
    for p in failfiles:
        ...
        for r in rr:
            if r['status']!='ok':raise SystemExit(f'fatal W10 FRAGSEG failure persisted: {r}')
    ...
    for p in files:
        ...
        if len(keys)!=1:raise SystemExit(f'cannot identify exactly one W10 viewer for shard {p}')
        key=next(iter(keys));seen.append(key)
        if rr and {r['segmenter_version'] for r in rr}!={VERSION}:raise SystemExit(f'invalid W10 segmenter version {p}')
        rows+=rr
    if set(seen)!=canonical or len(seen)!=len(canonical) or len(seen)!=len(set(seen)):raise SystemExit('W10 FRAGSEG viewer coverage mismatch')
    ...
    if pagekeys!=eligible:raise SystemExit(f'W10 eligible coverage mismatch missing={len(eligible-pagekeys)} extra={len(pagekeys-eligible)}')
    bypage=defaultdict(list)
    for r in rows:bypage[(r['viewer_key'],r['page_id'])].append(int(r['fragment_sequence']))
    gaprows=[]
    for (k,pid),vals in sorted(bypage.items()):
        seq=sorted(vals)
        if any(v<=0 for v in seq) or len(seq)!=len(set(seq)):raise SystemExit(f'invalid W10 fragment sequence {k} {pid}')
    ...
    if not rows:raise SystemExit('W10 FRAGSEG produced zero fragments')
    ...
    allc=summary[-1];slots=sum(int(r['missing_slot_count']) for r in gaprows);lines=['# FRAGSEG — LTMD-U1 W10 Integrados/Multiarea','',f'Versión: `{VERSION}`.','',f'- Identidades históricas preservadas: **{EXPECTED_IDENTITIES}/{EXPECTED_IDENTITIES}**.',f'- Identidades con fuente admitida: **{len(admitted)}**.',f'- Identidades retenidas: **{len(withheld)}**.',f'- Objetos canónicos: **{len(canonical)}**.',f'- Aliases byte-exactos: **{len(aliases)}**.',f'- Páginas elegibles PAGESTRUCT: **{len(eligible):,}**.',f'- Páginas con ≥1 fragmento: **{allc["segmented_page_count"]:,}**.',f'- Páginas elegibles sin fragmentos: **{len(empty)}**.',f'- Fragmentos: **{allc["fragment_count"]:,}**.',f'- IDs únicos: **{len(set(ids)):,}**.',f'- Páginas con huecos legítimos de secuencia: **{len(gaprows)}**.',f'- Slots omitidos: **{slots}**.','','## Tipos candidatos']
