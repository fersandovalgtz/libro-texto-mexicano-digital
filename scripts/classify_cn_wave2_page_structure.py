#!/usr/bin/env python3
"""Classify CN Wave 2 page structure with the same conservative CN46 logic."""
...
FLAGS=Path('data/expansion/cn_wave2_structural_keyword_flags.csv')
OUT=Path('data/expansion/cn_wave2_page_structure.csv')
SUMMARY=Path('data/expansion/cn_wave2_page_structure_summary.csv')
REPORT=Path('data/expansion/cn_wave2_page_structure_report.md')
VERSION='PAGESTRUCT_CN_WAVE2_0.1'
...
def main():
    ...
    if len(metrics)!=3177:raise SystemExit(f'expected 3177 Wave2 source pages, found {len(metrics)}')
    out=[]
    for r in metrics:
    ...
    lines=['# PAGESTRUCT — Ciencias Naturales Ola 2','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','', '## Total']
    for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
    lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','', '## Regla','Se conserva la misma lógica conservadora usada en CN4/CN6, agrupando por `book_id`. La clasificación estructural no constituye clasificación pedagógica ni `semantic_ready`.']
