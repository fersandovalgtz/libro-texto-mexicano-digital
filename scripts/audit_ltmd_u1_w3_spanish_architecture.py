...
SCOPE=Path('data/catalog/ltmd_u1_w3_scope.csv')
OUT=Path('data/catalog/ltmd_u1_w3_viewer_architecture.csv')
REPORT=Path('data/catalog/ltmd_u1_w3_viewer_architecture.md')
VERSION='LTMD_U1_W3_ARCHITECTURE_0.1'
UA='LibroTextoMexicanoDigital/U1-W3 architecture audit'
EXPECTED=130
...
def main():
    ...
    for i,s in enumerate(scope,1):
        ...
        if i%20==0:print(f'probed {i}/{EXPECTED}')
    ...
    lines=['# LTMD-U1 W3 — arquitectura de visores Español/Lengua','',f'Versión: `{VERSION}`.','',f'- Visores congelados: **{EXPECTED}**.',f'- HTML 200: **{html}/{EXPECTED}**.',f'- `x.js` 200: **{xok}/{EXPECTED}**.',f'- señal `ag_pages`: **{ag}/{EXPECTED}**.',f'- arquitectura dinámica estándar: **{standard}/{EXPECTED}**.',f'- casos no estándar: **{EXPECTED-standard}**.','','## Por generación','', '| generación | visores | estándar |','|---:|---:|---:|']
    for g,(n,st) in sorted(gens.items(),key=lambda x:int(x[0])):lines.append(f'| {g} | {n} | {st} |')
    lines+=['','Este probe no descarga páginas ni acredita disponibilidad de activos. Sólo verifica la arquitectura necesaria para una auditoría posterior de `claves.json` y JPEG fuente.']
