...
OUT=Path('data/catalog/ltmd_u1_w6_viewer_architecture.csv')
REPORT=Path('data/catalog/ltmd_u1_w6_viewer_architecture.md')
VERSION='LTMD_U1_W6_ARCHITECTURE_0.1'
UA='LibroTextoMexicanoDigital/U1-W6 Geography Atlas architecture audit'
EXPECTED=42
...
def main():
    ...
    lines=['# LTMD-U1 W6 — arquitectura de visores Geografía/Atlas','',f'Versión: `{VERSION}`.','',f'- Visores congelados: **{EXPECTED}**.',f'- HTML 200: **{html}/{EXPECTED}**.',f'- `x.js` 200: **{xok}/{EXPECTED}**.',f'- señal `ag_pages`: **{ag}/{EXPECTED}**.',f'- arquitectura dinámica estándar: **{standard}/{EXPECTED}**.',f'- casos no estándar: **{EXPECTED-standard}**.','','## Por generación','', '| generación | visores | estándar |','|---:|---:|---:|']
