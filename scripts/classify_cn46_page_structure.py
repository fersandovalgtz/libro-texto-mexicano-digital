#!/usr/bin/env python3
"""Classify page structure for CN4/CN6 expansion using book-aware flags."""
...
FLAGS=Path('data/expansion/cn46_structural_keyword_flags.csv')
OUT=Path('data/expansion/cn46_page_structure.csv')
SUMMARY=Path('data/expansion/cn46_page_structure_summary.csv')
REPORT=Path('data/expansion/cn46_page_structure_report.md')
VERSION='PAGESTRUCT_CN46_0.1'
...
def main():
    ...
    if len(metrics)!=1888:raise SystemExit(f'expected 1888 source pages, found {len(metrics)}')
    out=[]
    for r in metrics:
    ...
    allc=counts['ALL'];lines=['# PAGESTRUCT — expansión CN4/CN6','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','', '## Total']
    for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
    lines+=['','## Regla','Esta capa reutiliza la lógica PAGESTRUCT 0.2, pero corrige la unidad de agrupación: `book_id` en lugar de `catalog_generation`. Esto permite coexistencia de múltiples objetos dentro de una misma generación, como CN6/1993.']
    REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8')
