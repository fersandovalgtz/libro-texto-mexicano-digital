#!/usr/bin/env python3
"""Combine W7 Civics/Ethics FRAGSEG shards with exact coverage checks."""
from __future__ import annotations
import argparse,csv
from collections import Counter,defaultdict
from pathlib import Path
STRUCT=Path('data/catalog/ltmd_u1_w7_civics_ethics_page_structure.csv');OUT=Path('data/catalog/ltmd_u1_w7_civics_ethics_fragment_manifest.csv');SUMMARY=Path('data/catalog/ltmd_u1_w7_civics_ethics_fragment_manifest_summary.csv');GAPS=Path('data/catalog/ltmd_u1_w7_civics_ethics_fragment_sequence_gaps.csv');REPORT=Path('data/catalog/ltmd_u1_w7_civics_ethics_fragment_manifest.md');VERSION='FRAGSEG_LTMD_U1_W7_CIVICS_ETHICS_0.1';ELIGIBLE={'textual','mixed_text_image'};EXPECTED=25;EXPECTED_PAGES=3261

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w7_civics_ethics_fragments');a=ap.parse_args();structure=list(csv.DictReader(STRUCT.open(encoding='utf-8',newline='')));viewers={r['viewer_key'] for r in structure};eligible={(r['viewer_key'],r['page_id']) for r in structure if r['primary_structure'] in ELIGIBLE}
 if len(structure)!=EXPECTED_PAGES or len(viewers)!=EXPECTED:raise SystemExit(f'expected {EXPECTED_PAGES} W7 PAGESTRUCT rows/{EXPECTED} viewers, found {len(structure)}/{len(viewers)}')
 if {r['classifier_version'] for r in structure}!={'PAGESTRUCT_LTMD_U1_W7_CIVICS_ETHICS_0.1'}:raise SystemExit('unexpected W7 PAGESTRUCT version')
 ...
 for p in failfiles:
  ...
  for r in rr:
   if r['status']!='ok':raise SystemExit(f'fatal W7 FRAGSEG failure persisted: {r}')
 ...
 for p in files:
  ...
  if len(keys)!=1:raise SystemExit(f'cannot identify exactly one W7 viewer for shard {p}')
  key=next(iter(keys));seen.append(key)
  if rr and {r['segmenter_version'] for r in rr}!={VERSION}:raise SystemExit(f'invalid W7 segmenter version {p}')
  rows+=rr
 if set(seen)!=viewers or len(seen)!=EXPECTED or len(seen)!=len(set(seen)):raise SystemExit('W7 FRAGSEG viewer coverage mismatch')
 ...
 if pagekeys!=eligible:raise SystemExit(f'W7 eligible coverage mismatch missing={len(eligible-pagekeys)} extra={len(pagekeys-eligible)}')
 bypage=defaultdict(list)
 for r in rows:bypage[(r['viewer_key'],r['page_id'])].append(int(r['fragment_sequence']))
 gaprows=[]
 for (k,pid),vals in sorted(bypage.items()):
  seq=sorted(vals)
  if any(v<=0 for v in seq) or len(seq)!=len(set(seq)):raise SystemExit(f'invalid W7 fragment sequence {k} {pid}')
 ...
 if not rows:raise SystemExit('W7 FRAGSEG produced zero fragments')
 ...
 allc=summary[-1];slots=sum(int(r['missing_slot_count']) for r in gaprows);lines=['# FRAGSEG — LTMD-U1 W7 Cívica/Ética','',f'Versión: `{VERSION}`.','',f'- Objetos canónicos: **{EXPECTED}**.','- Identidades históricas retenidas por fuente: **5**.',f'- Páginas elegibles PAGESTRUCT: **{len(eligible):,}**.',f'- Páginas con ≥1 fragmento: **{allc["segmented_page_count"]:,}**.',f'- Páginas elegibles sin fragmentos: **{len(empty)}**.',f'- Fragmentos: **{allc["fragment_count"]:,}**.',f'- IDs únicos: **{len(set(ids)):,}**.',f'- Páginas con huecos legítimos de secuencia: **{len(gaprows)}**.',f'- Slots omitidos: **{slots}**.','','## Tipos candidatos']
 for t in types:lines.append(f'- `{t}`: {allc[t]:,}.')
 lines+=['','## Regla','Se reutiliza sin cambios el motor FRAGSEG W3 para comparabilidad técnica. Los tipos son candidatos; el texto completo no se persiste y cualquier fallo de fuente/SHA/OCR de ejecución hace fallar el shard. Las cinco identidades retenidas por fuente no se imputan ni se sustituyen. Esta capa no es validación semántica.']
