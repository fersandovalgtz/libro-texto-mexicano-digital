#!/usr/bin/env python3
"""OCR one canonical W10 Integrados/Multiarea viewer after the strict source gate.

The worker derives its allowed viewer set and page cardinality from the published
W10 processing topology. Source images are temporary, byte-size/SHA-256 verified,
and never persisted. Only technical OCR metrics are written.
"""
...
UA='LibroTextoMexicanoDigital/U1-W10 Integrated Multiarea OCR 0.1'
...
def load_topology():
    ...
    if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:
        raise SystemExit('W10 processing inventory cardinality mismatch')
    if {r['topology_version'] for r in proc}!={TOPOLOGY_VERSION}:
        raise SystemExit('W10 topology version mismatch')
    ...
    if not canonical or not admitted or not canonical<=admitted:
        raise SystemExit('W10 topology has no valid admitted canonical cohort')
    if any(r['processing_mode']!='direct_canonical' for r in proc if r['viewer_key'] in canonical):
        raise SystemExit('W10 canonical processing mode mismatch')
    if not man or {r['viewer_key'] for r in man}!=canonical:
        raise SystemExit('W10 canonical page manifest viewer mismatch')
    if {r['manifest_version'] for r in man}!={TOPOLOGY_VERSION}:
        raise SystemExit('W10 canonical manifest version mismatch')
    ids=[r['page_id'] for r in man]
    if len(ids)!=len(set(ids)):
        raise SystemExit('duplicate W10 canonical page IDs')
    if any(r['asset_status']!='source_jpeg' or r['processing_mode']!='direct_canonical' or not r['sha256'] or len(r['sha256'])!=64 or int(r['byte_size'])<=0 for r in man):
        raise SystemExit('W10 canonical page provenance mismatch')
    by={k:0 for k in canonical}
    for r in man:by[r['viewer_key']]+=1
    pmap={r['viewer_key']:r for r in proc}
    for k,n in by.items():
        if n!=int(pmap[k]['source_pages']):raise SystemExit(f'W10 source-page count mismatch {k}: {n}/{pmap[k]["source_pages"]}')
    return canonical
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w10_integrados_ocr');a=ap.parse_args();canonical=load_topology()
    if a.viewer_key not in canonical:raise SystemExit(f'viewer not W10 OCR-eligible canonical: {a.viewer_key}')
    ...
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w10-ocr-') as td:rows=[process(r,Path(td)) for r in source]
