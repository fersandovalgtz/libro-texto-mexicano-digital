...
INV=Path('data/catalog/ltmd_u1_w9_declared_inventory.csv')
ARCH=Path('data/catalog/ltmd_u1_w9_viewer_architecture.csv')
VERSION='LTMD_U1_W9_EDUCACION_FISICA_ASSET_AUDIT_0.1'
BASE='https://historico.conaliteg.gob.mx/c/{key}/{idx:03d}.jpg'
UA='LibroTextoMexicanoDigital/U1-W9 Physical Education asset audit'
EXPECTED=4
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w9_educacion_fisica_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W9 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W9: {a.viewer_key}')
    ...
    ui='standard_x_js' if ar['standard_dynamic_architecture']=='1' else 'nonstandard_viewer_architecture';records=[]
