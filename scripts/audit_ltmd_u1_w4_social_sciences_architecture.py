...
OUT=Path('data/catalog/ltmd_u1_w4_viewer_architecture.csv')
REPORT=Path('data/catalog/ltmd_u1_w4_viewer_architecture.md')
VERSION='LTMD_U1_W4_ARCHITECTURE_0.1'
UA='LibroTextoMexicanoDigital/U1-W4 Social Sciences architecture audit'
EXPECTED=14
...
def main():
    ...
    lines=['# LTMD-U1 W4 — arquitectura de visores Ciencias Sociales','',f'Versión: `{VERSION}`.','',f'- Visores congelados: **{EXPECTED}**.',f'- HTML 200: **{html}/{EXPECTED}**.',f'- `x.js` 200: **{xok}/{EXPECTED}**.',f'- señal `ag_pages`: **{ag}/{EXPECTED}**.',f'- arquitectura dinámica estándar: **{standard}/{EXPECTED}**.',f'- casos no estándar: **{EXPECTED-standard}**.','','## Por generación','', '| generación | visores | estándar |','|---:|---:|---:|']
    for g,(n,st) in sorted(gens.items(),key=lambda x:int(x[0])):lines.append(f'| {g} | {n} | {st} |')
    lines += ['', 'Este probe no acredita disponibilidad página por página. Sólo verifica la arquitectura necesaria para auditar `claves.json` y activos JPEG. OCR permanece cerrado hasta reconciliación fuente.']
