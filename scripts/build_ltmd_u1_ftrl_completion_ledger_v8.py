#!/usr/bin/env python3
"""Promote LTMD-U1 completion ledger 0.7 to 0.8 after verified W8 closure.

Metadata-only promotion. It consumes the canonical 0.7 ledger, the W8 Artes
source-admissibility gate, public text-free global evidence, and public archival
closure evidence. It promotes only the 16 source-admitted identities. The four
2018 source retentions remain blocked and are never aliased or imputed. This
script never reads or emits OCR text, private Drive identifiers, or key material.
"""
...
W8_GATE = Path("data/catalog/ltmd_u1_w8_artes_source_admissibility.csv")
W8_GLOBAL = Path("data/research/ltmd_u1_w8_global_evidence.json")
W8_CLOSURE = Path("data/research/ltmd_u1_w8_archival_closure.json")
...
OLD_VERSION = "LTMD_U1_FTRL_COMPLETION_LEDGER_0.7"
VERSION = "LTMD_U1_FTRL_COMPLETION_LEDGER_0.8"
SUMMARY_SCHEMA = "LTMD_U1_FTRL_COMPLETION_SUMMARY_0.8"
W8_RUN = "33220988830"
W8_COMMIT = "cae2bed8ff79b45675ae194a855abfed9b86aca4"
W8_ARCHIVE = "LTMD-U1 — corpus FTRL privado/W8 — Artes/run_33220988830__cae2bed__2026-08-28/02_CONSOLIDATED_PRIVATE"
...
def no_private_locator(value) -> None:
    if isinstance(value, dict):
        ...
        assert not (forbidden & set(value)), f"public W8 evidence exposes private material: {forbidden & set(value)}"
...
def validate_gate(rows: list[dict[str, str]]) -> tuple[dict[str, dict[str, str]], set[str]]:
    assert len(rows) == 20 and len({r["viewer_key"] for r in rows}) == 20
    admitted = [r for r in rows if int(r["source_admissible"]) == 1]
    withheld = [r for r in rows if int(r["source_admissible"]) == 0]
    assert len(admitted) == 16 and len(withheld) == 4
    assert {r["viewer_key"] for r in withheld} == WITHHELD
    assert all(r["source_status"] == "SOURCE_ADMISSIBLE" for r in admitted)
    assert all(int(r["direct_asset_ready"]) == 1 and int(r["internal_unserved"]) == 0 and int(r["probe_errors"]) == 0 for r in admitted)
    assert all(int(r["source_jpegs"]) == int(r["declared_positions"]) - 1 for r in admitted)
    assert all(r["alias_state"] == "no_alias" for r in admitted)
    assert all(r["source_status"] == "SOURCE_RETAINED" for r in withheld)
    assert all(int(r["source_jpegs"]) == 0 and int(r["direct_asset_ready"]) == 0 for r in withheld)
    assert all(int(r["internal_unserved"]) == int(r["declared_positions"]) and int(r["probe_errors"]) == 0 for r in withheld)
    assert all(r["alias_state"] == "no_alias" for r in withheld)
    assert sum(int(r["source_jpegs"]) for r in admitted) == 1490
    return {r["viewer_key"]: r for r in admitted}, WITHHELD


def validate_global(p: dict, admitted: dict[str, dict[str, str]]) -> dict[str, int]:
    assert p["schema"] == "LTMD_FTRL_W8_GLOBAL_EVIDENCE_0.1"
    assert p["wave"] == "W8" and p["status"] == "validated"
    assert p["historical_identities"] == 20 and p["canonical_processing_objects"] == 16
    assert p["withheld_source_identities"] == 4 and p["source_pages"] == 1490
    assert p["unique_page_key_hashes"] == 1490
    assert p["sqlite_page_rows"] == 1490 and p["fts_rows"] == 1490 and p["qc_page_records"] == 1490
    ...
    assert set(pages) == set(admitted) and sum(pages.values()) == 1490
    ...
    assert Counter(x["class"] for x in products) == Counter({"restricted_products": 48, "text_free_products": 48})
...
def validate_closure(p: dict) -> None:
    assert p["schema"] == "LTMD_FTRL_ARCHIVAL_CLOSURE_0.8"
    assert p["wave"] == "W8" and p["archival_complete"] is True
    f = p["ftrl"]
    assert f["canonical_processing_objects"] == 16 and f["historical_identities"] == 20 and f["withheld_source_identities"] == 4
    assert f["distributed_books"] == 16 and f["page_records"] == 1490 and f["fts_rows"] == 1490
    assert f["run_id"] == W8_RUN and f["commit"] == W8_COMMIT and f["status"] == "validated"
    ...
    assert a["encrypted_handoffs_preserved"] is True and a["encrypted_handoffs_unique"] == 16
    ...
    assert a["text_free_evidence_preserved"] is True and a["text_free_evidence_unique"] == 17
    assert a["consolidated_archive_bytes"] == 3090776
    assert a["consolidated_archive_sha256"] == "92a7dd0f8eb38c28c582954d947ac8c695020ef21118b13566e2702d51b937d4"
    assert a["logical_destination"] == W8_ARCHIVE
    q = p["qc"]
    assert q == {"page_records": 1490, "pages_flagged_for_technical_review": 317, "pages_unflagged": 1173, "zero_search_text_pages": 203}
    assert q["pages_flagged_for_technical_review"] + q["pages_unflagged"] == 1490
    sr = p["source_retention"]
    assert sr["count"] == 4 and set(sr["viewer_keys"]) == WITHHELD and sr["aliases_introduced"] == 0
...
def validate_prior_w7(rows: list[dict[str, str]]) -> None:
...
def validate_base(rows: list[dict[str, str]], admitted: set[str]) -> set[str]:
    ...
    validate_prior_w7(rows)
    w8 = [r for r in rows if r["wave"] == "W8"]
    assert len(w8) == 20 and {r["viewer_key"] for r in w8} == admitted | WITHHELD
    retained = [r for r in w8 if r["viewer_key"] in WITHHELD]
    ...
    proc = [r for r in w8 if r["viewer_key"] in admitted]
...
def promote(rows: list[dict[str, str]], gate: dict[str, dict[str, str]], pages: dict[str, int]) -> list[dict[str, str]]:
    ...
    for original in rows:
        ...
        if viewer in gate:
            g = gate[viewer]
            assert row["wave"] == "W8" and row["documentary_disposition"] == "required_ftrl_processing"
            ...
            row["ftrl_run_id"] = W8_RUN
            row["ftrl_commit"] = W8_COMMIT
            row["corpus_ready"] = "1"
            row["ocr_available"] = "1"
            row["text_verified"] = "0"
            ...
            row["archive_destination_logical"] = W8_ARCHIVE
            row["interpretive_limit"] = "Computational/archival closure of source-admitted W8 only; retained identities remain excluded; OCR is not human text verification or semantic evidence."
        out.append(row)
    return out
...
def build_summary(rows: list[dict[str, str]]) -> dict:
    ...
    validate_prior_w7(rows)
    w8 = [r for r in rows if r["wave"] == "W8"]
    assert Counter(r["ftrl_status"] for r in w8) == Counter({"validated": 16, "blocked_active_retention": 4})
    assert Counter(r["archival_status"] for r in w8) == Counter({"archival_complete": 16, "not_started": 4})
    assert sum(r["is_canonical_processing_object"] == "1" for r in w8) == 16
    assert sum(int(r["canonical_source_pages"] or 0) for r in w8 if r["is_canonical_processing_object"] == "1") == 1490
    assert sum(int(r["corpus_ready"]) for r in w8) == 16 and sum(int(r["ocr_available"]) for r in w8) == 16
    assert sum(int(r["text_verified"]) for r in w8) == 0 and sum(int(r["semantic_ready"]) for r in w8) == 0
    canonical = Counter()
    pages = Counter()
    for r in rows:
    ...
    assert validated == 289 and terminal == 294 and remaining == 248 and pending == 235
    assert canonical == Counter({"validated": 261}) and pages == Counter({"validated": 42805})
    assert archival == Counter({"archival_complete": 289, "not_applicable_final_exception": 5, "not_started": 248})
    return {
...
def main() -> None:
    ...
    args = ap.parse_args()
    gate, _ = validate_gate(read_csv(W8_GATE))
    global_evidence = json.loads(W8_GLOBAL.read_text(encoding="utf-8"))
    pages = validate_global(global_evidence, gate)
    validate_closure(json.loads(W8_CLOSURE.read_text(encoding="utf-8")))
    rows = read_csv(args.input)
    validate_base(rows, set(gate))
    rows = promote(rows, gate, pages)
