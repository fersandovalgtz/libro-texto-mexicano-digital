#!/usr/bin/env python3
"""Classify canonical W5 History pages with conservative PAGESTRUCT logic."""
from __future__ import annotations
import csv
from collections import Counter,defaultdict
from pathlib import Path
METRICS=Path('data/catalog/ltmd_u1_w5_history_ocr_metrics.csv');FLAGS=Path('data/catalog/ltmd_u1_w5_history_structural_keyword_flags.csv');OUT=Path('data/catalog/ltmd_u1_w5_history_page_structure.csv');SUMMARY=Path('data/catalog/ltmd_u1_w5_history_page_structure_summary.csv');REPORT=Path('data/catalog/ltmd_u1_w5_history_page_structure.md');VERSION='PAGESTRUCT_LTMD_U1_W5_HISTORY_0.1';EXPECTED=15;EXPECTED_PAGES=2653
...
def main():
 ...
 if len(metrics)!=EXPECTED_PAGES or len({r['viewer_key'] for r in metrics})!=EXPECTED:raise SystemExit('W5 PAGESTRUCT OCR coverage mismatch')
 out=[]
 for r in metrics:
 ...
 allc=counts['ALL'];eligible=allc['textual']+allc['mixed_text_image'];lines=['# PAGESTRUCT — LTMD-U1 W5 Historia','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','',f'Objetos canónicos: **{EXPECTED}**.','', '## Total']
 for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
 lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','','## Regla','Se conserva la lógica PAGESTRUCT conservadora. Esta capa es estructural, no semántica. Los tres visores 2018 permanecen cubiertos como aliases de provenance hacia sus pares 2019; no se duplican páginas. La ausencia de referencia humana no impide PAGESTRUCT/FRAGSEG, pero sí impide tratar clasificadores semánticos no validados como evidencia histórica primaria.']
