#!/usr/bin/env python3
"""SHA-verified adaptive OCR metrics for one canonical LTMD-U1 W5 History viewer."""
...
UA='LibroTextoMexicanoDigital/U1-W5 History OCR 0.1'
...
def load_topology():
    ...
    if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:raise SystemExit('W5 processing inventory cardinality mismatch')
    if any(r['technical_identity_covered']!='1' for r in proc):raise SystemExit('W5 contains uncovered technical identity')
    canonical={r['viewer_key'] for r in proc if r['is_canonical_processing_object']=='1'}
    if len(canonical)!=EXPECTED_CANONICAL:raise SystemExit('W5 topology must contain exactly 15 canonical processing objects')
    if any(r['processing_mode']!='direct_canonical' for r in proc if r['viewer_key'] in canonical):raise SystemExit('W5 canonical has unexpected processing mode')
    aliases=[r for r in proc if r['processing_mode']=='route_alias_to_2019']
    if len(aliases)!=3 or any(r['is_canonical_processing_object']!='0' for r in aliases):raise SystemExit('W5 route-alias topology mismatch')
    if any(int(r['persistent_unresolved_source_gaps'] or 0)!=0 for r in proc):raise SystemExit('W5 persistent source gap')
    if len(man)!=EXPECTED_SOURCE_PAGES or {r['viewer_key'] for r in man}!=canonical:raise SystemExit('W5 canonical page manifest cardinality/coverage mismatch')
    if any(r['asset_status']!='source_jpeg' for r in man):raise SystemExit('W5 canonical page manifest contains non-source row')
    pids=[page_id(r) for r in man]
    if len(pids)!=len(set(pids)):raise SystemExit('duplicate W5 canonical page IDs')
    return canonical
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w5_history_ocr');a=ap.parse_args();canonical=load_topology()
    if a.viewer_key not in canonical:raise SystemExit(f'viewer not W5 canonical: {a.viewer_key}')
    source=[r for r in csv.DictReader(MAN.open(encoding='utf-8',newline='')) if r['viewer_key']==a.viewer_key]
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w5-history-ocr-') as td:rows=[process(r,Path(td)) for r in source]
