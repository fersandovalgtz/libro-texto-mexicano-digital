#!/usr/bin/env python3
"""Run one deterministic private LTMD-U1 W10 FTRL book unit.

Restricted OCR/SQLite/QC stay under local/ and must be encrypted before any
Actions upload. The source topology is rebuilt from the current byte-revalidated
W10 gate. H2014P1ENA remains outside the productive corpus as a final exception.
"""
...
EXPECTED_TOTAL = 11937
FINAL_EXCEPTION = 'H2014P1ENA'
SCHEMA = 'LTMD_FTRL_W10_BOOK_UNIT_0.1'
    ...
    with path.open(encoding='utf-8') as fh:
        for line_number, line in enumerate(fh, 1):
            if not line.strip():
                continue
            row = json.loads(line)
            if row.get('wave') != 'W10' or row.get('viewer_key') != viewer_key:
                raise SystemExit(f'foreign W10 record at {path}:{line_number}')
...
def main() -> None:
    ...
    ap.add_argument('--output-dir', type=Path, default=Path('local/ftrl-w10'))
    args = ap.parse_args()
    require_environment()
    ...
    full_asset = root / 'ltmd_u1_w10_asset_manifest.csv'
    processing = root / 'ltmd_u1_w10_processing_inventory.csv'
    run([sys.executable, 'scripts/build_ftrl_w10_inputs.py', '--asset-output', str(full_asset), '--processing-output', str(processing)])

    proc = read_csv(processing)
    if len(proc) != EXPECTED_CANONICAL:
        raise SystemExit('W10 canonical processing denominator drift')
    ...
    if len(canonical) != EXPECTED_CANONICAL or viewer not in canonical:
        raise SystemExit(f'viewer is not a W10 source-admitted canonical identity: {viewer}')
    if FINAL_EXCEPTION in canonical:
        raise SystemExit('W10 final exception leaked into private FTRL corpus')
    if any(r['persistent_internal_source_gaps'] != '0' for r in proc):
        raise SystemExit('W10 admitted processing inventory contains a source gap')
    ...
    if len(assets) != EXPECTED_TOTAL:
        raise SystemExit('W10 global source cardinality drift')
    if len({r['viewer_key'] for r in assets}) != EXPECTED_CANONICAL:
        raise SystemExit('W10 source-admitted viewer denominator drift')
    rows = sorted([r for r in assets if r['viewer_key'] == viewer], key=lambda r: int(r['source_image_index']))
    if len(rows) != pages_expected:
        raise SystemExit(f'W10 {viewer} source cardinality drift: {len(rows)} != {pages_expected}')
    ...
    if len(expected_hashes) != len(set(expected_hashes)):
        raise SystemExit('duplicate expected page key inside W10 book unit')

    prefix = f'w10_{viewer}'
    ...
    run_network_resilient([sys.executable, 'scripts/build_page_ocr_corpus.py', '--asset-manifest', str(unit_asset), '--processing-inventory', str(processing), '--wave', 'W10', '--output', str(jsonl), '--cache-dir', str(unit / 'assets'), '--resume'])
    ...
    if actual_count != pages_expected or actual_hashes != expected_hashes:
        raise SystemExit('W10 book page inventory mismatch')
    ...
    if rm['status'] != 'validated' or rm['database']['sqlite_integrity'] != 'ok':
        raise SystemExit('W10 book validation failed')
    if rm['corpus']['page_records'] != pages_expected or rm['database']['page_rows'] != pages_expected or rm['database']['fts_rows'] != pages_expected or qc['page_records'] != pages_expected:
        raise SystemExit('W10 book cardinality drift')

    payload = {
        'schema': SCHEMA,
        'status': 'validated',
        'wave': 'W10',
        'domain': 'Integrados/Multiarea',
        'viewer_key': viewer,
        ...
        'source_partition': {
            ...
            'final_exception_identities': EXPECTED_FINAL_EXCEPTION,
        ...
        'technical_reuse_reference': 'data/research/ltmd_u1_w10_technical_reuse_audit.json',
        'epistemic_guards': [
            'computationally_validated != archival_complete',
            'ocr_available != text_verified',
            'corpus_ready != semantic_ready',
            'final_exception_identity != alias_candidate',
