...
DECL=Path('data/catalog/ltmd_u1_w6_declared_inventory.csv')
OUT=Path('data/catalog/ltmd_u1_w6_geography_atlas_asset_manifest.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w6_geography_atlas_asset_summary.csv')
REPORT=Path('data/catalog/ltmd_u1_w6_geography_atlas_asset_audit.md')
VERSION='LTMD_U1_W6_GEOGRAPHY_ATLAS_ASSET_AUDIT_0.1'
EXPECTED=42

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w6_geography_atlas_assets');a=ap.parse_args()
    ...
    if len(scope)!=EXPECTED or len(decl)!=EXPECTED or len(files)!=EXPECTED:raise SystemExit(f'W6 scope/declared/files mismatch {len(scope)}/{len(decl)}/{len(files)}')
    rows=[];seen=[]
    for p in files:
        ...
        if len(ks)!=1 or vs!={VERSION}:raise SystemExit(f'invalid W6 shard {p}')
        key=next(iter(ks));seen.append(key);expected=int(decl[key]['declared_positions'])
        if len(rr)!=expected:raise SystemExit(f'W6 cardinality mismatch {key}: {len(rr)} vs {expected}')
        if any(r['probe_state']=='probe_error' or r['asset_status']=='probe_error' for r in rr):raise SystemExit(f'W6 probe error persisted {key}')
        if {r['ag_clave'] for r in rr}!={decl[key]['ag_clave']}:raise SystemExit(f'W6 ag_clave drift {key}')
        rows+=rr
    if set(seen)!=set(scope) or len(seen)!=len(set(seen)):raise SystemExit('W6 viewer coverage/duplicate mismatch')
    if len(rows)!=sum(int(r['declared_positions']) for r in decl.values()):raise SystemExit('W6 total declared-position mismatch')
    ...
    lines += ['', '## Regla', 'Cada byte servido se recorre sólo para SHA-256 y tamaño; no se persisten JPEG. Un 404 final se conserva como candidato terminal y un 404 interno como anomalía. `direct_asset_ready` es un estado técnico de fuente y no acredita independencia documental, continuidad histórica ni equivalencia curricular.', '', 'La excepción de arquitectura HTML se conserva explícitamente y no se normaliza de forma ficticia. Coincidencias de título, grado, año o cardinalidad no autorizan aliases. OCR W6 permanece cerrado hasta analizar relaciones exactas entre activos, routing y huecos.']
