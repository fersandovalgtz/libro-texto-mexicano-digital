#!/usr/bin/env python3
"""Audit one LTMD-U1 W7 Civics/Ethics viewer from institutional ag_clave.
...
INV=Path('data/catalog/ltmd_u1_w7_declared_inventory.csv')
...
UA='LibroTextoMexicanoDigital/U1-W7 Civics Ethics asset audit'
EXPECTED=30
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w7_civics_ethics_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W7 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W7: {a.viewer_key}')
