#!/usr/bin/env python3
"""Validate exact union of all W10 Integrados/Multiarea FTRL book evidences."""
...
SCHEMA = "LTMD_FTRL_W10_GLOBAL_EVIDENCE_0.1"
...
def main() -> None:
    ...
    if len(proc) != EXPECTED_CANONICAL:
        raise SystemExit(f"expected {EXPECTED_CANONICAL} W10 processing rows, found {len(proc)}")
    viewers = {r["viewer_key"] for r in proc}
    if len(viewers) != EXPECTED_CANONICAL:
        raise SystemExit("duplicate W10 canonical processing identity")
    if not all(r["technical_identity_covered"] == "1" and r["is_canonical_processing_object"] == "1" and r["persistent_internal_source_gaps"] == "0" for r in proc):
        raise SystemExit("invalid W10 admitted processing inventory")
    if len(assets) != EXPECTED_TOTAL:
        raise SystemExit(f"expected {EXPECTED_TOTAL} W10 source pages, found {len(assets)}")
    ...
    if counts != expected_counts:
        raise SystemExit(f"W10 per-viewer page counts drift: {counts} != {expected_counts}")

    expected_hashes = {page_key_hash(r["viewer_key"], int(r["source_image_index"])) for r in assets}
    if len(expected_hashes) != EXPECTED_TOTAL:
        raise SystemExit("W10 expected page-key hash collision or duplicate")

    evidence_files = sorted(args.evidence_root.rglob("w10_*_evidence.json"))
    if len(evidence_files) != EXPECTED_CANONICAL:
        raise SystemExit(f"expected {EXPECTED_CANONICAL} W10 evidence files, found {len(evidence_files)}")
    ...
    for path in evidence_files:
        ...
        if data["schema"] != "LTMD_FTRL_W10_BOOK_UNIT_0.1":
            raise SystemExit(f"unexpected W10 book evidence schema: {path}")
        if data["status"] != "validated" or data["wave"] != "W10":
            raise SystemExit(f"non-validated or foreign W10 evidence: {path}")
        viewer = data["viewer_key"]
        if viewer not in viewers or viewer in seen_viewers:
            raise SystemExit(f"unexpected or duplicate W10 viewer evidence: {viewer}")
        seen_viewers.add(viewer)

        pages = int(data["page_records"])
        if pages != expected_counts[viewer]:
            raise SystemExit(f"W10 evidence page drift for {viewer}: {pages}")
        hashes = list(data["page_key_hashes"])
        if len(hashes) != pages or len(set(hashes)) != pages:
            raise SystemExit(f"duplicate/missing W10 page hashes for {viewer}")
        if union_hashes & set(hashes):
            raise SystemExit(f"cross-book W10 page hash overlap for {viewer}")
        ...
        if int(partition["historical_identities"]) != EXPECTED_HISTORICAL or int(partition["canonical_processing_objects"]) != EXPECTED_CANONICAL or int(partition["withheld_identities"]) != EXPECTED_WITHHELD or int(partition["full_admitted_source_pages"]) != EXPECTED_TOTAL:
            raise SystemExit(f"W10 source partition drift for {viewer}")
        ...
        if int(validation["sqlite_pages"]) != pages or int(validation["fts_rows"]) != pages or int(validation["qc_page_records"]) != pages:
            raise SystemExit(f"W10 validation cardinality drift for {viewer}")
        ...
        for group in ("restricted_products", "text_free_products"):
            if len(data[group]) != 3:
                raise SystemExit(f"unexpected W10 product count for {viewer}: {group}")
            for item in data[group]:
                if len(item["sha256"]) != 64 or int(item["bytes"]) <= 0:
                    raise SystemExit(f"invalid W10 product descriptor for {viewer}")
    ...
    if seen_viewers != viewers:
        raise SystemExit("W10 evidence viewer union is not exhaustive")
    if union_hashes != expected_hashes or len(union_hashes) != EXPECTED_TOTAL:
        raise SystemExit("W10 page evidence union is not exact and exhaustive")
    if sqlite_records != EXPECTED_TOTAL or fts_records != EXPECTED_TOTAL or qc_records != EXPECTED_TOTAL:
        raise SystemExit("W10 global SQLite/FTS/QC cardinality drift")
    if len(product_hashes) != EXPECTED_CANONICAL * 6:
        raise SystemExit("W10 global product descriptor cardinality drift")
    ...
    out = {
        ...
        "withheld_viewer_keys": ["H2014P1ENA"],
