#!/usr/bin/env python3
"""Per-book, SHA-verified FRAGSEG for LTMD-U1 W1 1966."""
...
STRUCTURE=Path('data/catalog/ltmd_u1_w1_1966_page_structure.csv')
MANIFEST=Path('data/catalog/ltmd_u1_w1_1966_page_manifest.csv')
VERSION='FRAGSEG_LTMD_U1_W1_1966_0.1';ELIGIBLE={'textual','mixed_text_image'};UA='LibroTextoMexicanoDigital/U1-W1 1966 FRAGSEG'
...
def process_page(r,src,temp):
    ...
    return fragments,'ok'
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--book-id',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w1_1966_fragments');args=ap.parse_args()
    ...
    allfrags=[];failures=[]
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w1-1966-frag-') as td:
        temp=Path(td)
        for r in structure:
    ...
    d=Path(args.output_dir);d.mkdir(parents=True,exist_ok=True);slug=args.book_id.lower().replace('u1-','');out=d/f'fragment_{slug}.csv';fail=d/f'fragment_{slug}_failures.csv'
