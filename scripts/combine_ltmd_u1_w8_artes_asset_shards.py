...
DECL=Path('data/catalog/ltmd_u1_w8_declared_inventory.csv')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w8_artes_assets');a=ap.parse_args()
    ...
    if len(scope)!=EXPECTED or len(decl)!=EXPECTED or len(files)!=EXPECTED:raise SystemExit(f'W8 scope/declared/files mismatch {len(scope)}/{len(decl)}/{len(files)}')
    rows=[];seen=[]
    for p in files:
        ...
        if len(ks)!=1 or vs!={VERSION}:raise SystemExit(f'invalid W8 shard {p}')
        key=next(iter(ks));seen.append(key)
        if key not in decl:raise SystemExit(f'viewer outside W8 scope: {key}')
