#!/usr/bin/env python3
"""Audit one W3 Spanish/Language viewer using its institutional ag_clave.

Prepared for later execution. It streams source bytes only for SHA-256 and never
persists page images. HTTP 404 states are recorded rather than silently removed.
"""
...
ARCH=Path('data/catalog/ltmd_u1_w3_viewer_architecture.csv')
VERSION='LTMD_U1_W3_SPANISH_ASSET_AUDIT_0.1'
BASE='https://historico.conaliteg.gob.mx/c/{key}/{idx:03d}.jpg'
UA='LibroTextoMexicanoDigital/U1-W3 Spanish asset audit'
EXPECTED=130
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w3_spanish_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W3 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W3: {a.viewer_key}')
    ...
    ui='standard_x_js' if ar['standard_dynamic_architecture']=='1' else 'horizontal_x_horizontal_js'
    records=[]
    ...
    print(f"{a.viewer_key}: ui={ui} ag_clave={key} declared={n} served={c['source_jpeg']} terminal404={c['terminal_synthetic_candidate']} internal404={c['internal_unserved']} probe_error={c['probe_error']}")
