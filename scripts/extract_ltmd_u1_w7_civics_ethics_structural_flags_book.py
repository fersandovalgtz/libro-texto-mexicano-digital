#!/usr/bin/env python3
"""Extract conservative structural keyword flags for one canonical LTMD-U1 W7 Civics/Ethics viewer."""
from __future__ import annotations
import argparse,csv,hashlib,os,re,subprocess,tempfile,unicodedata
from pathlib import Path
from urllib.request import Request,urlopen

METRICS=Path('data/catalog/ltmd_u1_w7_civics_ethics_ocr_metrics.csv')
MAN=Path('data/catalog/ltmd_u1_w7_canonical_page_manifest.csv')
VERSION='LTMD_U1_W7_CIVICS_ETHICS_STRUCTKW_0.1'
UA='LibroTextoMexicanoDigital/U1-W7 Civics Ethics structural flags 0.1'
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w7_civics_ethics_structkw');a=ap.parse_args()
 metrics_all=list(csv.DictReader(METRICS.open(encoding='utf-8',newline='')))
 if len(metrics_all)!=EXPECTED_PAGES or len({r['viewer_key'] for r in metrics_all})!=EXPECTED_CANONICAL:raise SystemExit('W7 structural flags require complete 25-canonical OCR layer')
 metrics=[r for r in metrics_all if r['viewer_key']==a.viewer_key]
 if not metrics:raise SystemExit(f'no W7 OCR rows for {a.viewer_key}')
 if any(r['source_sha256_verified']!='1' or r['ocr_status']!='ok' for r in metrics):raise SystemExit(f'invalid W7 OCR rows for {a.viewer_key}')
 ...
 with tempfile.TemporaryDirectory(prefix='ltmd-u1-w7-ce-struct-') as td:
  td=Path(td)
