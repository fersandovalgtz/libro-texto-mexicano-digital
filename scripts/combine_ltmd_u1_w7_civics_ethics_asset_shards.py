...
SCOPE=Path('data/catalog/ltmd_u1_w7_scope.csv')
DECL=Path('data/catalog/ltmd_u1_w7_declared_inventory.csv')
OUT=Path('data/catalog/ltmd_u1_w7_civics_ethics_asset_manifest.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w7_civics_ethics_asset_summary.csv')
REPORT=Path('data/catalog/ltmd_u1_w7_civics_ethics_asset_audit.md')
VERSION='LTMD_U1_W7_CIVICS_ETHICS_ASSET_AUDIT_0.1'
EXPECTED=30

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input-dir',default='data/work/ltmd_u1_w7_civics_ethics_assets');a=ap.parse_args()
    ...
    if len(scope)!=EXPECTED or len(decl)!=EXPECTED or len(files)!=EXPECTED:raise SystemExit(f'W7 scope/declared/files mismatch {len(scope)}/{len(decl)}/{len(files)}')
    rows=[];seen=[]
    for p in files:
        ...
        if len(ks)!=1 or vs!={VERSION}:raise SystemExit(f'invalid W7 shard {p}')
        key=next(iter(ks));seen.append(key);expected=int(decl[key]['declared_positions'])
        if len(rr)!=expected:raise SystemExit(f'W7 cardinality mismatch {key}: {len(rr)} vs {expected}')
        if any(r['probe_state']=='probe_error' or r['asset_status']=='probe_error' for r in rr):raise SystemExit(f'W7 probe error persisted {key}')
        if {r['ag_clave'] for r in rr}!={decl[key]['ag_clave']}:raise SystemExit(f'W7 ag_clave drift {key}')
        rows+=rr
    if set(seen)!=set(scope) or len(seen)!=len(set(seen)):raise SystemExit('W7 viewer coverage/duplicate mismatch')
    if len(rows)!=sum(int(r['declared_positions']) for r in decl.values()):raise SystemExit('W7 total declared-position mismatch')
