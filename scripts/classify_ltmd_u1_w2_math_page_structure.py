#!/usr/bin/env python3
"""Classify W2 Mathematics page structure using the frozen conservative CN logic."""
...
def main():
 ...
 if len({r['viewer_key'] for r in metrics})!=64:raise SystemExit('W2 PAGESTRUCT requires 64 viewers')
 out=[]
 for r in metrics:
 ...
 allc=counts['ALL'];eligible=allc['textual']+allc['mixed_text_image'];lines=['# PAGESTRUCT — LTMD-U1 W2 Matemáticas','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','', '## Total']
 for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
 lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','', '## Regla','Se conserva la lógica conservadora usada en CN4/CN6, Ola 2 y W1. Esta clasificación estructural no constituye clasificación pedagógica ni `semantic_ready`.']
