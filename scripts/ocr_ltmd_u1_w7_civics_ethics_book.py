#!/usr/bin/env python3
"""SHA-verified adaptive OCR metrics for one canonical LTMD-U1 W7 Civics/Ethics viewer."""
...
UA='LibroTextoMexicanoDigital/U1-W7 Civics Ethics OCR 0.1'
...
def load_topology():
    ...
    if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:raise SystemExit('W7 processing inventory cardinality mismatch')
    ...
    if canonical!=eligible or len(canonical)!=EXPECTED_CANONICAL:raise SystemExit('W7 topology must contain exactly 25 eligible canonicals')
    if any(r['processing_mode']!='direct_canonical' for r in proc if r['viewer_key'] in canonical):raise SystemExit('W7 admitted viewer has unexpected processing mode')
    if any(int(r['persistent_internal_source_gaps'] or 0)!=0 for r in proc if r['viewer_key'] in canonical):raise SystemExit('W7 admitted viewer has persistent source gap')
    withheld=[r for r in proc if r['viewer_key'] not in canonical]
    if len(withheld)!=5 or any(r['processing_mode']!='withheld_source' or r['ocr_identity_eligible']!='0' for r in withheld):raise SystemExit('W7 withheld topology mismatch')
    if len(man)!=EXPECTED_SOURCE_PAGES or {r['viewer_key'] for r in man}!=canonical:raise SystemExit('W7 canonical page manifest cardinality/coverage mismatch')
    if any(r['asset_status']!='source_jpeg' for r in man):raise SystemExit('W7 canonical page manifest contains non-source row')
    pids=[page_id(r) for r in man]
    if len(pids)!=len(set(pids)):raise SystemExit('duplicate W7 canonical page IDs')
    return canonical
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w7_civics_ethics_ocr');a=ap.parse_args();canonical=load_topology()
    if a.viewer_key not in canonical:raise SystemExit(f'viewer not W7 OCR-eligible canonical: {a.viewer_key}')
    source=[r for r in csv.DictReader(MAN.open(encoding='utf-8',newline='')) if r['viewer_key']==a.viewer_key]
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w7-ce-ocr-') as td:rows=[process(r,Path(td)) for r in source]
