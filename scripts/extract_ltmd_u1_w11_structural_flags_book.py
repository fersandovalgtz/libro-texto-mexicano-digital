#!/usr/bin/env python3
"""Extract conservative structural keyword flags for one canonical W11 viewer."""
...
VERSION='LTMD_U1_W11_STRUCTKW_0.1'
UA='LibroTextoMexicanoDigital/U1-W11 structural flags 0.1'
VOCAB={
...
def download_verify(row:dict[str,str],target:Path)->None:
...
def run_ocr(image:Path,psm:int)->str:
...
def main()->None:
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w11_structkw');a=ap.parse_args()
    metrics_all=list(csv.DictReader(METRICS.open(encoding='utf-8',newline='')))
    if not metrics_all or len({r['page_id'] for r in metrics_all})!=len(metrics_all):raise SystemExit('W11 structural flags require complete unique OCR layer')
    viewers={r['viewer_key'] for r in metrics_all};manifest_all=list(csv.DictReader(MAN.open(encoding='utf-8',newline='')))
    if {r['viewer_key'] for r in manifest_all}!=viewers or len(manifest_all)!=len(metrics_all):raise SystemExit('W11 OCR/manifest coverage mismatch')
    if any(r['source_sha256_verified']!='1' or r['ocr_status']!='ok' for r in metrics_all):raise SystemExit('W11 structural flags refuse unresolved OCR')
    metrics=[r for r in metrics_all if r['viewer_key']==a.viewer_key]
    if not metrics:raise SystemExit(f'no W11 OCR rows for {a.viewer_key}')
    ...
    with tempfile.TemporaryDirectory(prefix='ltmd-u1-w11-struct-') as td:
        td=Path(td)
        for row in candidates:
            src=manifest[(a.viewer_key,int(row['viewer_page']))];image=td/f"{row['page_id']}.jpg";text='';verified=0;error=''
            try:download_verify(src,image);verified=1;text=norm(run_ocr(image,int(row['selected_psm'] or 3)))
