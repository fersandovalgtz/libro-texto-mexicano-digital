...
OUT=Path('data/catalog/ltmd_u1_w3_declared_inventory.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w3_declared_inventory_summary.csv')
REPORT=Path('data/catalog/ltmd_u1_w3_declared_inventory.md')
VERSION='LTMD_U1_W3_DECLARED_INVENTORY_0.1'
EXPECTED=130
UA='LibroTextoMexicanoDigital/U1-W3 declared inventory'

def main():
    with QUEUE.open(encoding='utf-8',newline='') as f: scope=[r for r in csv.DictReader(f) if r['operational_domain']=='espanol_lengua']
    if len(scope)!=EXPECTED:raise SystemExit(f'expected {EXPECTED} W3 viewers, got {len(scope)}')
    ...
    for s in scope:
        ...
        pages=d.get('ag_pages')
        try:n=int(pages)
        except Exception:raise SystemExit(f'invalid ag_pages {s["viewer_key"]}: {pages!r}')
    ...
    sr=[]
    for g,(n,p) in sorted(by.items(),key=lambda x:int(x[0])):sr.append({'inventory_version':VERSION,'catalog_generation':g,'viewer_count':n,'declared_positions':p})
    ...
    total=sr[-1]['declared_positions'];lines=['# LTMD-U1 W3 — inventario declarado Español/Lengua','',f'Versión: `{VERSION}`.','',f'- Visores: **{EXPECTED}**.',f'- Posiciones declaradas por `claves.json`: **{total:,}**.','','## Por generación','', '| generación | visores | posiciones declaradas |','|---:|---:|---:|']
    for r in sr[:-1]:lines.append(f"| {r['catalog_generation']} | {r['viewer_count']} | {int(r['declared_positions']):,} |")
    lines+=['','Este inventario no presupone que cada posición tenga un JPEG real, ni que el último folio sea sintético. La capa de activos deberá probarlo empíricamente por objeto.']
