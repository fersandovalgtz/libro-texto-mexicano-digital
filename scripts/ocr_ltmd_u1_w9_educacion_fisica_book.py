...
MAN=Path('data/catalog/ltmd_u1_w9_canonical_page_manifest.csv');PROC=Path('data/catalog/ltmd_u1_w9_processing_inventory.csv')
VERSION='LTMD_U1_W9_EDUCACION_FISICA_OCR_0.1';UA='LibroTextoMexicanoDigital/U1-W9 Physical Education OCR 0.1';FALLBACK_MIN_WORDS=5;TIMEOUT=60;EXPECTED_IDENTITIES=4;EXPECTED_CANONICAL=4;EXPECTED_SOURCE_PAGES=448
...
def load_topology():
 ...
 if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:raise SystemExit('W9 processing inventory cardinality mismatch')
 canonical={r['viewer_key'] for r in proc if r['is_canonical_processing_object']=='1'};eligible={r['viewer_key'] for r in proc if r['ocr_identity_eligible']=='1'}
 if canonical!=eligible or len(canonical)!=EXPECTED_CANONICAL:raise SystemExit('W9 topology must contain exactly 4 OCR-eligible canonicals')
 if any(r['processing_mode']!='direct_canonical' for r in proc):raise SystemExit('W9 unexpected processing mode')
 ...
 if any(r['asset_status']!='source_jpeg' or not r['sha256'] or len(r['sha256'])!=64 for r in man):raise SystemExit('W9 canonical page provenance mismatch')
 return canonical
...
def run_ocr(image,psm):
 env=os.environ.copy();env['OMP_THREAD_LIMIT']='1';cp=subprocess.run(['tesseract',str(image),'stdout','-l','spa','--psm',str(psm),'tsv'],capture_output=True,text=True,timeout=TIMEOUT,env=env)
...
def process(row,tmp):
 pid=row['page_id'];image=tmp/f'{pid}.jpg';attempts=[];errors=[];base={'ocr_version':VERSION,'page_id':pid,'viewer_key':row['viewer_key'],'catalog_generation':row['catalog_generation'],'grade':row['grade_code'],'title_core':row['title_core'],'viewer_page':row['viewer_page'],'source_image_index':row['source_image_index'],'processing_mode':row['processing_mode'],'source_provenance':row['source_provenance']};empty={'recognized_words':'','ocr_chars':'','mean_word_confidence':'','median_word_confidence':'','low_confidence_word_rate':''}
 try:
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w9_educacion_fisica_ocr');a=ap.parse_args();canonical=load_topology()
 if a.viewer_key not in canonical:raise SystemExit(f'viewer not W9 OCR-eligible canonical: {a.viewer_key}')
 source=[r for r in csv.DictReader(MAN.open(encoding='utf-8',newline='')) if r['viewer_key']==a.viewer_key]
 with tempfile.TemporaryDirectory(prefix='ltmd-u1-w9-ef-ocr-') as td:rows=[process(r,Path(td)) for r in source]
