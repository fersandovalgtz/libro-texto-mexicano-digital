#!/usr/bin/env python3
"""Audit LTMD-U1 W5 History 2018 routing anomalies against paired 2019 routes.

The three 2018 viewer identities remain catalog records in their own right. This
script tests only whether each anomalous 2018 entry can be resolved operationally
through the corresponding 2019 content route. It requires matched grade, normalized
title and declared cardinality, then re-fetches every paired 2019 JPEG and compares
live SHA-256 and byte size with the persisted W5 2019 reference manifest. Source
bytes are streamed and discarded.
"""
...
MANIFEST = Path('data/catalog/ltmd_u1_w5_history_asset_manifest.csv')
OUT = Path('data/catalog/ltmd_u1_w5_history_2018_2019_route_identity.csv')
REL = Path('data/catalog/ltmd_u1_w5_history_2018_2019_route_relationships.csv')
REPORT = Path('data/catalog/ltmd_u1_w5_history_2018_2019_route_identity.md')
VERSION = 'LTMD_U1_W5_HISTORY_2018_2019_ROUTE_IDENTITY_0.1'
UA = 'LibroTextoMexicanoDigital/U1-W5 History 2018-2019 route identity'
EXPECTED_ANOMALOUS_VIEWERS = 3
...
def main():
    readiness = list(csv.DictReader(READINESS.open(encoding='utf-8')))
    manifest = list(csv.DictReader(MANIFEST.open(encoding='utf-8')))
    state_by_key = {r['viewer_key']: r for r in readiness}
    man_by_key = {}
    for row in manifest:
    ...
    anomalies = [
        ...
        if r['catalog_generation'] == '2018' and r['source_state'] == 'no_source_jpegs'
    ]
    if len(anomalies) != EXPECTED_ANOMALOUS_VIEWERS:
        raise SystemExit(f'expected {EXPECTED_ANOMALOUS_VIEWERS} W5 2018 routing anomalies, got {len(anomalies)}')
    ...
    for a in sorted(anomalies, key=lambda r: r['viewer_key']):
        ...
        checks = {
            ...
            'paired_full_direct': b['source_state'] == 'full_direct_source',
        ...
        unresolved = sorted(int(r['viewer_page']) for r in rows18 if r['asset_status'] == 'internal_unserved')
        if unresolved != sorted(refs):
            raise SystemExit(
                f'{key18}->{key19}: 2018 unresolved pages do not exactly match 2019 source-page set '
                f'({len(unresolved)} vs {len(refs)})'
            )
    ...
    for a, b, refs in pairs:
        ...
        relationships.append({
            ...
            'relationship_type': 'catalog_entry_resolves_through_paired_2019_asset_route' if complete else 'paired_route_identity_not_proven',
            'canonical_processing_viewer_key': b['viewer_key'] if complete else '',
            'interpretive_limit': 'Operational route resolution only. Catalog identities remain distinct; this does not assign a bibliographic edition year to the 2018 entry.',
        })
    ...
    lines = [
        '# LTMD-U1 W5 — resolución de rutas Historia 2018 ↔ 2019', '',
        f'Versión: `{VERSION}`.', '',
    ...
    lines += ['', '## Interpretación',
        'La resolución completa permite reutilizar operacionalmente el contenido canónico servido bajo la clave 2019 sin duplicar OCR. Las entradas 2018 y 2019 se conservan como identidades institucionales separadas. La evidencia es de enrutamiento y bytes digitales; no demuestra por sí sola identidad bibliográfica, fecha de edición ni continuidad curricular.'
    ]
    REPORT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(REPORT.read_text(encoding='utf-8'))

    if complete_viewers != len(relationships) or sha_ok != total or size_ok != total:
        raise SystemExit('one or more W5 History 2018/2019 route pairs remain unresolved')
