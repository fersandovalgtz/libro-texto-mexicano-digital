...
OUT=Path('data/catalog/ltmd_u1_w4_declared_inventory.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w4_declared_inventory_summary.csv')
REPORT=Path('data/catalog/ltmd_u1_w4_declared_inventory.md')
VERSION='LTMD_U1_W4_DECLARED_INVENTORY_0.1'
EXPECTED=14
UA='LibroTextoMexicanoDigital/U1-W4 Social Sciences declared inventory'

def main():
    scope=list(csv.DictReader(SCOPE.open(encoding='utf-8',newline='')))
    if len(scope)!=EXPECTED:raise SystemExit(f'expected {EXPECTED} W4 viewers, got {len(scope)}')
    ...
    total=int(sr[-1]['declared_positions']);lines=['# LTMD-U1 W4 — inventario declarado Ciencias Sociales','',f'Versión: `{VERSION}`.','',f'- Visores: **{EXPECTED}**.',f'- Posiciones declaradas por `claves.json`: **{total:,}**.','','## Por generación','', '| generación | visores | posiciones declaradas |','|---:|---:|---:|']
    for r in sr[:-1]:lines.append(f"| {r['catalog_generation']} | {r['viewer_count']} | {int(r['declared_positions']):,} |")
    lines += ['', 'Este inventario no acredita que cada posición declarada corresponda a un JPEG servido. La auditoría de activos debe probar cada posición y conservar explícitamente terminales sintéticos, huecos internos y errores de probe.']
