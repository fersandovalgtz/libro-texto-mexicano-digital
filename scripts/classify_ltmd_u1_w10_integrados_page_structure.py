#!/usr/bin/env python3
"""Classify canonical W10 pages with the frozen conservative PAGESTRUCT logic."""
...
FLAGS=Path('data/catalog/ltmd_u1_w10_integrados_structural_keyword_flags.csv')
PROC=Path('data/catalog/ltmd_u1_w10_processing_inventory.csv')
OUT=Path('data/catalog/ltmd_u1_w10_integrados_page_structure.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w10_integrados_page_structure_summary.csv')
REPORT=Path('docs/LTMD_U1_W10_PAGESTRUCT.md')
VERSION='PAGESTRUCT_LTMD_U1_W10_INTEGRADOS_0.1'
...
def main():
    ...
    if len(proc)!=EXPECTED_IDENTITIES or len({r['viewer_key'] for r in proc})!=EXPECTED_IDENTITIES:raise SystemExit('W10 PAGESTRUCT topology cardinality mismatch')
    canonical={r['viewer_key'] for r in proc if r['source_admitted']=='1' and r['is_canonical_processing_object']=='1'}
    admitted={r['viewer_key'] for r in proc if r['source_admitted']=='1'};withheld={r['viewer_key'] for r in proc if r['source_admitted']=='0'};aliases={r['viewer_key'] for r in proc if r['processing_mode']=='exact_source_alias'}
    if not metrics or {r['viewer_key'] for r in metrics}!=canonical or len({r['page_id'] for r in metrics})!=len(metrics):raise SystemExit('W10 PAGESTRUCT OCR coverage mismatch')
    if any(r['source_sha256_verified']!='1' or r['ocr_status']!='ok' for r in metrics):raise SystemExit('W10 PAGESTRUCT refuses unverified/unresolved OCR rows')
    out=[]
    for r in metrics:
    ...
    lines=['# PAGESTRUCT — LTMD-U1 W10 Integrados/Multiarea','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','',f'Identidades históricas preservadas: **{EXPECTED_IDENTITIES}/{EXPECTED_IDENTITIES}**.',f'Identidades con fuente admitida: **{len(admitted)}**.',f'Identidades retenidas: **{len(withheld)}**.',f'Objetos canónicos: **{len(canonical)}**.',f'Aliases byte-exactos: **{len(aliases)}**.','','## Total']
    for cl in CLASSES:lines.append(f'- `{cl}`: {allc[cl]:,}.')
    lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','','## Regla','Se reutiliza sin cambios la lógica PAGESTRUCT conservadora de W4/W7/W8/W9 para mantener comparabilidad técnica. W10 es multiarea: esta capa sólo describe estructura de página y no introduce una taxonomía semántica nueva. `WAITING_HUMAN_REFERENCE` continúa vigente.']
