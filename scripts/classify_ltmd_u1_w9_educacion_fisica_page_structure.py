#!/usr/bin/env python3
"""Classify canonical W9 Educación Física pages with the frozen conservative PAGESTRUCT logic."""
from __future__ import annotations
import csv
from collections import Counter,defaultdict
from pathlib import Path

METRICS=Path('data/catalog/ltmd_u1_w9_educacion_fisica_ocr_metrics.csv')
FLAGS=Path('data/catalog/ltmd_u1_w9_educacion_fisica_structural_keyword_flags.csv')
OUT=Path('data/catalog/ltmd_u1_w9_educacion_fisica_page_structure.csv')
SUMMARY=Path('data/catalog/ltmd_u1_w9_educacion_fisica_page_structure_summary.csv')
REPORT=Path('data/catalog/ltmd_u1_w9_educacion_fisica_page_structure.md')
VERSION='PAGESTRUCT_LTMD_U1_W9_EDUCACION_FISICA_0.1'
EXPECTED=4
EXPECTED_PAGES=448
...
def main():
    ...
    if len(metrics)!=EXPECTED_PAGES or len({r['viewer_key'] for r in metrics})!=EXPECTED:
        raise SystemExit('W9 PAGESTRUCT OCR coverage mismatch')
    if any(r['source_sha256_verified']!='1' or r['ocr_status']!='ok' for r in metrics):
        raise SystemExit('W9 PAGESTRUCT refuses unverified/unresolved OCR rows')
    out=[]
    for r in metrics:
    ...
    for r in out:
        counts[r['viewer_key']][r['primary_structure']]+=1;counts['ALL'][r['primary_structure']]+=1
    ...
    lines=['# PAGESTRUCT — LTMD-U1 W9 Educación Física','',f'Versión: `{VERSION}`. Páginas clasificadas: **{len(out):,}**.','',f'Objetos canónicos: **{EXPECTED}**.','Identidades retenidas por fuente: **0**.','','## Total']
    for cl in classes:lines.append(f'- `{cl}`: {allc[cl]:,}.')
    lines+=['',f'Páginas elegibles para FRAGSEG (`textual` + `mixed_text_image`): **{eligible:,}**.','','## Regla','Se conserva sin cambios la lógica PAGESTRUCT conservadora empleada en W4/W7/W8 para mantener comparabilidad técnica. Esta capa es estructural, no semántica. Las cuatro identidades W9 cuentan con fuente admitida y SHA-256 verificado. La ausencia de referencia humana no impide PAGESTRUCT/FRAGSEG, pero sí impide tratar clasificadores semánticos no validados como evidencia histórica primaria.']
