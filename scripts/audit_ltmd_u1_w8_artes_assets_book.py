#!/usr/bin/env python3
"""Audit one LTMD-U1 W8 Arts viewer from institutional ag_clave.

Source bytes are streamed only to compute SHA-256 and byte size; they are not
persisted. This is a technical source-integrity layer, not a semantic or
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--viewer-key',required=True);ap.add_argument('--output-dir',default='data/work/ltmd_u1_w8_artes_assets');a=ap.parse_args()
    ...
    if len(inv)!=EXPECTED or len(arch)!=EXPECTED:raise SystemExit('W8 inventory/architecture cardinality mismatch')
    if a.viewer_key not in inv:raise SystemExit(f'viewer not in W8: {a.viewer_key}')
